//! Solve a lower-bounded dense linear least-squares problem with `DBOLS`.
//!
//! Required features: `std`, one native backend, and
//! `least-squares-linear-bounded`. This minimizes a residual subject to a
//! closed variable bound; it is not linear programming.

// slatec-safe-example: bounded linear least squares
...
fn main() -> Result<(), slatec::bounded_least_squares::BoundedLeastSquaresError> {
    ...
    assert!((fit.solution[0] - 0.0).abs() < 1.0e-12);
    assert!((fit.solution[1] - 2.0).abs() < 1.0e-12);
    assert!((fit.residual_norm - 1.0).abs() < 1.0e-12);
    Ok(())
}
