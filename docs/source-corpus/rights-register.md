...
| `RIGHT-017` | Hash-pinned scalar release source scope (91 exact FNLIB and SLATEC `src` units) | `source-specific explicit public domain` | NIST's SLATEC programme report and its NTIS distribution record each explicitly describe the SLATEC library as public domain; the Netlib SLATEC Version 4.1 index establishes the exact `fnlib` and `src` subset scope. The committed evidence registry binds the scalar-family union to its deterministic source-set SHA-256, count, and family closures. | The target carrier may redistribute only exact units in the reviewed source-set receipt, with generated provenance, runtime, and redistribution notices. This does not clear any other SLATEC source or imported package. |
