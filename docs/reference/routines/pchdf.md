...
- Canonical provider: `pchip/pchdf.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/pchip/pchdf.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/pchip/pchdf.f) — `verified_cached`
...
| `X` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (K) | PCHDF: PCHIP Finite Difference Formula Uses a divided difference formulation to compute a K-point approximation to the derivative at X(K) based on the data in X and S. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `S` | output | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (K) | PCHDF: PCHIP Finite Difference Formula Uses a divided difference formulation to compute a K-point approximation to the derivative at X(K) based on the data in X and S. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
