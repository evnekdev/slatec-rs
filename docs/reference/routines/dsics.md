...
- Exact Netlib source: [DSICS](https://www.netlib.org/slatec/lin/dsics.f)
...
| 10 | `EL` | `output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (NEL) | Double Precision EL(NEL). the unit lower triangular factor of the incomplete decomposition of the A matrix stored in SLAP Row format. The Diagonal of ones *IS* stored. See "Description", below for more details about the SLAP Row fmt. |
| 11 | `D` | `output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision D(N) Upon return this array holds D(I) = 1. /DIAG(A). |
| 12 | `R` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision R(N). Temporary double precision workspace needed for the factorization. |
