...
| 10 | `EL` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (NEL) | OUT      Double Precision EL(NEL). contain the unit lower triangular factor  of the incomplete decomposition   of the A  matrix  stored  in  SLAP Row format.   The Diagonal of   ones   *IS*   stored.     See "Description", below for more details about the SLAP Row fmt. |
| 11 | `D` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | trans, of a symmetric positive definite matrix, A, which is stored in SLAP Column format.  The unit lower triangular matrix L is stored by rows, and the inverse of the diagonal matrix D is stored. OUT      Double Precision D(N) 1./DIAG(A). |
| 12 | `R` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | WORK     Double Precision R(N). Temporary double precision workspace needed for the factorization. |
