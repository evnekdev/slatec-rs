...
| 5 | `A` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (NELT) | tion goes well.  It is set to the row index corresponding to the last zero pivot found.  See "Description", below. |
...
| 7 | `NEL` | `input-output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Scalar argument classified by fixed-form executable read/write analysis. |
| 8 | `IEL` | `input-output` | `array` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (NEL) | Array argument classified by fixed-form executable read/write analysis. |
| 9 | `JEL` | `input-output` | `array` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (NEL) | Array argument classified by fixed-form executable read/write analysis. |
| 10 | `EL` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (NEL) | Array argument classified by fixed-form executable read/write analysis. |
| 11 | `D` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | 1./DIAG(A). R      :WORK     Double Precision R(N). Temporary double precision workspace needed for the factorization. IWARN  :OUT      Integer. |
| 12 | `R` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Array argument classified by fixed-form executable read/write analysis. |
| 13 | `IWARN` | `input-output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Scalar argument classified by fixed-form executable read/write analysis. |
