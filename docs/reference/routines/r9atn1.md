...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `REAL` (`implicit_rule`) | `*mut f32` | scalar | Evaluate ATAN(X) from first order, that is, evaluate (ATAN(X)-X)/X**3 with relative error accuracy so that ATAN(X) = X + X**3*R9ATN1(X). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
