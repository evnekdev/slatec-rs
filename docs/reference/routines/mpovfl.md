...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | rank 1; dimensions (*) | when the exponent of 'mp' number X would exceed M. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
