...
| 7 | `A` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | (input) the limits of integration. NOTE: There is no requirement that [A,B] be contained in [X(1),X(N)]. However, the resulting integral value will be highly suspect, if not. |
| 8 | `B` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | (input) the limits of integration. NOTE: There is no requirement that [A,B] be contained in [X(1),X(N)]. However, the resulting integral value will be highly suspect, if not. |
