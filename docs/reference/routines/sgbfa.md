...
- Canonical provider: `lin/sgbfa.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/sgbfa.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/lin/sgbfa.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `ABD` | input/output | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (LDA, *) | On Entry ABD REAL(LDA, N) contains the matrix in band storage. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `LDA` | input | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | On Entry ABD REAL(LDA, N) contains the matrix in band storage. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `N` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | On Entry ABD REAL(LDA, N) contains the matrix in band storage. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
- Canonical Rust path: `slatec_sys::linear_algebra::banded::sgbfa`
- Compatibility aliases: `slatec_sys::linear_algebra::dense::sgbfa`
