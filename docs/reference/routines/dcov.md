...
| 4 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 2. 3. is a positive integer input variable set to the number of variables.  N must not exceed M. contain the value at which the covariance matrix is to be evaluated.  This is usually the value for X returned from a successful run of DNLS1 (or DNLS1E).  The value of X will not be changed. contains the covariance contains the covariance matrix evaluated at X. matrix evaluated at X. contains the QR contains the QR factorization of the Jacobian (probably not of factorization of the Jacobian (probably not of interest to the user). interest to the user). |
| 5 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are available for printing in DNLS1 or DNLS1E. IFLAG will never be zero when FCN is called by DCOV. must not be changed. RETURN contain the value contain the value at which the covariance matrix is to be evaluated.  This is at which the covariance matrix is to be evaluated.  This is usually the value for X returned from a successful run of usually the value for X returned from a successful run of DNLS1 (or DNLS1E).  The value of X will not be changed. DNLS1 (or DNLS1E).  The value of X will not be changed. |
| 6 | `FVEC` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are available for printing in DNLS1 or DNLS1E. IFLAG will never be zero when FCN is called by DCOV. must not be changed. RETURN contains the functions evaluated at X. |
| 7 | `R` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (LDR, *) | 1 and 2, R is an M by N array. contains the covariance matrix evaluated at X. 1 and 2, contains the QR factorization of the Jacobian (probably not of interest to the user). |
...
| 10 | `WA1` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are work arrays of length N. and WA3 |
| 11 | `WA2` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are work arrays of length N. and WA3 |
| 12 | `WA3` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | is a work array of length N. |
| 13 | `WA4` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | is a work array of length M. |
