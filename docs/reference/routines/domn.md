...
| 21 | `P` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (N, 0:NSAVE) | NSAVE), AP(N,0:NSAVE), EMAP(N,0:NSAVE) DOUBLE PRECISION DZ(N), CSAV(NSAVE), RWORK(USER DEFINED) EXTERNAL MATVEC, MSOLVE CALL DOMN(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, $     NSAVE, ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, R, $     Z, P, AP, EMAP, DZ, CSAV, RWORK, IWORK) < epsilon**2. WORK     Double Precision P(N,0:NSAVE). |
| 22 | `AP` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (N, 0:NSAVE) | < epsilon**2. WORK     Double Precision AP(N,0:NSAVE). |
| 23 | `EMAP` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (N, 0:NSAVE) | WORK     Double Precision EMAP(N,0:NSAVE). |
| 24 | `DZ` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | WORK     Double Precision DZ(N). |
| 25 | `CSAV` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (NSAVE) | WORK     Double Precision CSAV(NSAVE) Double Precision arrays used for workspace. |
