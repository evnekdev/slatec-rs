...
| `YH` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (N, *) | If the caller has changed H, YH must be rescaled. | If the caller has changed H, YH must be rescaled. Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `A` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (MATDIM, *) | If a failure occurs (in corrector convergence or error test), RMAX is set at RMFAIL for the next increase. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
