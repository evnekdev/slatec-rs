...
| 4 | `BI` | `input-output` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | Bi(z) on ID=0 dBi/dz on ID=1 at z=Z =2 returns exp(abs(Re(zeta)))*Bi(z) on ID=0 exp(abs(Re(zeta)))*dBi/dz on ID=1 at z=Z where zeta=(2/3)*z**(3/2) Result of type COMPLEX. |
