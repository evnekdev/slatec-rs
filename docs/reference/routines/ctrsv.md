...
| 7 | `X` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (*) | b,   or   A'*x = b,   or   conjg( A')*x = b, is an n by n unit, or non-unit, upper or lower triangular matrix. No test for singularity or near-singularity is included in this routine. Such tests must be performed before calling this routine. Parameters ========== COMPLEX          array of dimension at least must not be zero. Unchanged on exit. |
