...
| `ICALL` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | ICALL - Integer If DQAWOE is to be used only once, ICALL must be set to 1. | ICALL - Integer If DQAWOE is to be used only once, ICALL must be set to 1. Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
