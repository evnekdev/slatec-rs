...
| 2 | `ALPHA` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | order of first member of the sequence, ALPHA .GE. 0.0D0 1)/(X), K=1,...,N 1)/(X) or scaled values for EXP(-X)*I/sub(ALPHA+K-1)/(X), K=1,...,N depending on KODE |
...
| 4 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | number of members in the sequence, N .GE. 1 Output     Y is double precision NZ+1,...,N. |
| 5 | `Y` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | 1)/(X), K=1,...,N X)*I/sub(ALPHA+K-1)/(X), K=1,...,N a vector whose first N components contain NZ+1,...,N. |
