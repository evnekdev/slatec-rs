...
- Exact Netlib source: [SSGS](https://www.netlib.org/slatec/lin/ssgs.f)
...
| 16 | `RWORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | WORK     Real RWORK(LENW). Real array used for workspace. NL+3*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). |
| 17 | `LENW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer. NL+3*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). |
