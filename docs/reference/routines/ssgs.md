...
| 16 | `RWORK` | `workspace-output` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | RWORK(LENW). array used for workspace. |
| 17 | `LENW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Integer. Length of the real workspace, RWORK. LENW >= NL+3*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). |
