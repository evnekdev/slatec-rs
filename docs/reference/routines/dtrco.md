...
| `RCOND` | output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | On Return RCOND DOUBLE PRECISION an estimate of the reciprocal condition of T . | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
