...
| 2 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision X(N). The vector that should be multiplied by the matrix. |
| 3 | `Y` | `output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | A*X. Double Precision Y(N). The product of the matrix and the vector. |
