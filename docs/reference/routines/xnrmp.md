...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `NU` | input | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | The order MU and degree NU are non-negative integers and the argument is real. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `MU1` | input | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | The input values to XNRMP are NU, MU1, MU2, SARG, and MODE. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `MU2` | input | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | The input values to XNRMP are NU, MU1, MU2, SARG, and MODE. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `SARG` | input | `REAL` (`explicit`) | `*mut f32` | scalar | The input values to XNRMP are NU, MU1, MU2, SARG, and MODE. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `MODE` | input | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | The input values to XNRMP are NU, MU1, MU2, SARG, and MODE. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `SPN` | output | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | The output of XNRMP consists of the two vectors SPN and IPN and the error estimate ISIG. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `IPN` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | rank 1; dimensions (*) | The output of XNRMP consists of the two vectors SPN and IPN and the error estimate ISIG. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `ISIG` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | The output of XNRMP consists of the two vectors SPN and IPN and the error estimate ISIG. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `IERROR` | input | `INTEGER` (`implicit_rule`) | `*mut crate::FortranInteger` | scalar | As an example of usage, the Fortran coding J = K DO 20 I = 1, K CALL XCON(SPN(I), IPN(I),IERROR) IF (IERROR.NE.0) RETURN PRINT 10, SPN(I), IPN(I) 10 FORMAT(1X, E30.8 , I15) IF ((IPN(I) .EQ. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |

The table reports compiler/interface facts separately from source-prologue semantics. Unknown intent, aliasing, workspace, leading-dimension, and retention rules remain explicit; parameter names alone are never treated as semantic evidence. Native code does not retain ordinary argument pointers unless a reviewed declaration explicitly says otherwise.

### ABI and safety

Canonical path: `slatec_sys::special::xnrmp`. Native symbol: `xnrmp_`. Feature: `special`. Provider status: `selected_provider_verified`. ABI fingerprint: `subroutine:void(mut_i32,mut_i32,mut_i32,mut_f32,mut_i32,mut_f32_ptr_rank1,mut_i32_ptr_rank1,mut_i32,mut_i32)`. Every pointer must be aligned and valid for the full source-defined readable or writable extent; callers must uphold array dimensions, leading dimensions, workspace formulas, aliasing restrictions, callback lifetimes, and process-global runtime serialization.
...
- Compatibility aliases: `slatec_sys::special::numerical::xnrmp`
