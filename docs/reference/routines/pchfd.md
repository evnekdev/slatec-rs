...
| 5 | `INCFD` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | ...) INTEGER  N, NE, IERR REAL  X(N), F(INCFD,N), D(INCFD,N), XE(NE), FE(NE), DE(NE) LOGICAL  SKIP CALL  PCHFD (N, X, F, D, INCFD, SKIP, NE, XE, FE, DE, IERR) Parameters: (input) increment between successive values in F and D. |
