...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `L2` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Parameter in 3j symbol. GE. ABS(M2) and L3. ABS(M3); 2. L2+ABS(M2) and L3+ABS(M3) must be integers;. |
| 2 | `L3` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Parameter in 3j symbol. |
| 3 | `M2` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Parameter in 3j symbol. |
| 4 | `M3` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Parameter in 3j symbol. |
| 5 | `L1MIN` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | Smallest allowable L1 in 3j symbol. must be a non-negative integer, where. |
| 6 | `L1MAX` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | Largest allowable L1 in 3j symbol. must be a non-negative integer, where L2+L3 and L1MIN=MAX(ABS(L2-L3),ABS(M2+M3)). If the conventional restrictions are satisfied, then these restrictions are met. The user should be cautious in using input parameters that do not satisfy the conventional restrictions. For example, the the subroutine produces values of f(L1) = ( L1 2. 5 5. |
| 7 | `THRCOF` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (NDIM) | Set of 3j coefficients generated by evaluating the 3j symbol for all allowed values of L1. THRCOF(I) will contain f(L1MIN+I-1), I=1,2,. ,L1MAX+L1MIN+1. |
