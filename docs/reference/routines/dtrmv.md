...
- Safe Rust paths: `slatec::blas::level2::dtrmv`

## Providers

- Canonical provider: `lin/dtrmv.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/dtrmv.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/lin/dtrmv.f) — `verified_cached`
...
- Exact Netlib source: [DTRMV](https://www.netlib.org/slatec/lin/dtrmv.f)
...
| 5 | `A` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (LDA, *) | unit, upper or lower triangular matrix. Parameters ========== is unit triangular as follows: DOUBLE PRECISION array of DIMENSION ( LDA, n). is not referenced. is not referenced. are not referenced either, but are assumed to be unity. Unchanged on exit. |
...
Canonical Rust path: `slatec_sys::blas::level2::dtrmv`. Native symbol: `dtrmv_`. Declaration feature: `blas-level2`. Provider feature: `blas-level2`. ABI fingerprint: `unavailable`.
...
- Safe-wrapper status: `slatec::blas::level2::dtrmv`
