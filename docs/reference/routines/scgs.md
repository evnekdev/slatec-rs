...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer Order of the Matrix. hand side vector, and Z is the solution upon return.  NELT,  IA, JA, A is the number of columns in the matrix and |
| 2 | `B` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | IN       Real B(N). Right-hand side vector. |
| 3 | `X` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | INOUT    Real X(N). On input X is your initial guess for solution vector. On output X is the final approximate solution. B. |
...
| 16 | `IERR` | `input-output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 4.) OUT      Integer. Return error flag. 0 => All went well. 1 => Insufficient space allocated for WORK or IWORK. 2 => Method failed to converge in ITMAX steps. 3 => Error in user input. Check input values of N, ITOL. 4 => User error tolerance set too tight. Reset to 500*R1MACH(3).  Iteration proceeded. 5 => Breakdown of the method detected. 6 => Stagnation of the method detected. |
...
| 18 | `R` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | hand side vector, and Z is the solution upon return.  NELT,  IA, JA, A B. approximately 0. WORK     Real R(N). |
| 19 | `R0` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | approximately 0. approximately 0. WORK     Real R0(N). |
| 20 | `P` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | WORK     Real P(N). |
| 21 | `Q` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | WORK     Real Q(N). |
| 22 | `U` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | WORK     Real U(N). |
| 23 | `V1` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | WORK     Real V1(N). |
| 24 | `V2` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | WORK     Real V2(N). Real arrays used for workspace. |
| 25 | `RWORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | and IWORK arrays).   The name  of  the MSOLVE routine must be declared  external  in the  calling   program.   The WORK     Real RWORK(USER DEFINED). Real array that can be used for workspace in MSOLVE. |
