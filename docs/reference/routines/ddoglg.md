...
- Canonical provider: `main-src/src/ddoglg.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/ddoglg.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/ddoglg.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/ddoglg.f) — `verified_cached`
...
| `R` | input | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (*) | That is, if A = Q*R, where Q has orthogonal columns and R is an upper triangular matrix, then DDOGLG expects the full upper triangle of R and the first N components of (Q transpose)*B. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `X` | input | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (*) | Given an M by N matrix A, an N by N nonsingular diagonal matrix D, an M-vector B, and a positive number DELTA, the problem is to determine the convex combination X of the Gauss-Newton and scaled gradient directions that minimizes (A*X - B) in the least squares sense, subject to the restriction that the Euclidean norm of D*X be at most DELTA. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
