...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | Evaluate LOG(1+X) from 2-nd order with relative error accuracy so that LOG(1+X) = X - X**2/2 + X**3*D9LN2R(X) Series for LN21 on the interval -6.25000E-01 to 0. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
