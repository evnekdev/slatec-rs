...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `NPEROD` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Indicates the values which X(I,0) and X(I,N+1) are assumed to have. = 1 If X(I,0) = -X(I,1) and X(I,N+1) = -X(I,N) = 2 If X(I,0) = -X(I,1) and X(I,N+1) =  X(I,N) = 3 If X(I,0) =  X(I,1) and X(I,N+1) =  X(I,N) |
| 2 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | X(I,N) The number of unknowns in the J-direction.  N must be greater than 2. are listed are listed in the table below.  More comprehensive timing in the table below.  More comprehensive timing charts may be found in the reference. charts may be found in the reference. To measure the accuracy of the algorithm a To measure the accuracy of the algorithm a uniform random number generator was used to create uniform random number generator was used to create |
| 3 | `MPEROD` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 0 the array elements must not depend on the index I, but must be constant. Specifically, the subroutine checks the following condition 1 |
...
| 11 | `W` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | contains the required length of W. Long Description: * * * * * *   Program Specifications    * * * * * * * * * * * * Dimension of   A(M),B(M),C(M),Y(IDIMY,N), Arguments      W(see argument list) Latest         June 1, 1977 Revision Subprograms    POISTG,POSTG2,COSGEN,MERGE,TRIX,TRI3,PIMACH Required Special        NONE Conditions Common         NONE Blocks I/O            NONE Precision      Single Specialist     Roland Sweet Language       FORTRAN |
