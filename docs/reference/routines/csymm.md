...
| 11 | `C` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 2; dimensions (LDC, *) | = alpha*A*B + beta*C, or = alpha*B*A + beta*C, where alpha and beta are scalars, A is a symmetric matrix and B and are m by n matrices. COMPLEX array of DIMENSION ( LDC, n ). Before entry, the leading m by n part of the array C must contain the matrix C, except when beta is zero, in which case C need not be set on entry. On exit, the array C is overwritten by the m by n updated matrix. |
