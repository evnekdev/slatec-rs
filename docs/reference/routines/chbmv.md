...
| 4 | `ALPHA` | `input` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | are scalars, x and y are n element vectors and COMPLEX         . On entry, ALPHA specifies the scalar alpha. Unchanged on exit. |
| 5 | `A` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 2; dimensions (LDA, *) | diagonals. Parameters ========== must satisfy  0 .le. K. Unchanged on exit. COMPLEX          array of DIMENSION ( LDA, n ). contain the upper triangular band part of the hermitian matrix, supplied column by column, with the leading diagonal of the matrix in row matrix( I, J ) 10    CONTINUE 20 CONTINUE contain the lower triangular band part of the hermitian matrix, supplied column by column, with the leading diagonal of the matrix in row 1 of the array, the first sub-diagonal starting at position 1 in row 2, and so on. The bottom right k by k triangle of the array A is not referenced. The following program segment will transfer the lower triangular part of a hermitian band matrix from conventional full matrix storage to band storage: DO 20, J = 1, N M = 1 - J DO 10, I = J, MIN( N, J + K ) matrix( I, J ) 10    CONTINUE 20 CONTINUE Note that the imaginary parts of the diagonal elements need not be set and are assumed to be zero. Unchanged on exit. |
...
| 7 | `X` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (*) | COMPLEX          array of DIMENSION at least must not be zero. Unchanged on exit. |
...
| 9 | `BETA` | `input` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | are scalars, x and y are n element vectors and COMPLEX         . On entry, BETA specifies the scalar beta. Unchanged on exit. |
| 10 | `Y` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (*) | = alpha*A*x + beta*y, COMPLEX          array of DIMENSION at least is overwritten by the updated vector y. is overwritten by the updated vector y. must not be zero. Unchanged on exit. |
