...
| 2 | `B` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | B(N). Right hand side. |
| 3 | `X` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | X(N). Solution of (L*D*U)trans x = b. |
...
| 6 | `L` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | L(NL). the unit lower triangular factor of the incomplete decomposition of some matrix stored in SLAP Row format. The diagonal of ones *IS* stored. This structure can be set up by the SSILUS routine. See the "Description", below for more details about the SLAP format. (NL is the number of non-zeros in the L array. |
| 7 | `DINV` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | DINV(N). Inverse of the diagonal matrix D. |
...
| 10 | `U` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | U(NU). the unit upper triangular factor of the incomplete decomposition of some matrix stored in SLAP Column format. The diagonal of ones *IS* stored. This structure can be set up by the SSILUS routine. See the "Description", below for more details about the SLAP format. (NU is the number of non-zeros in the U array. |
