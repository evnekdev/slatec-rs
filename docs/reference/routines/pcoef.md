...
| 3 | `TC` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Vector containing the first LL+1 Taylor coefficients where LL=ABS(L).  If  L.GT.0 , the coefficients are in the usual Taylor series order, i.e. C) + ... + TC(N+1)*(X-C)**N C) + ... + TC(N+1)*(X-C)**N If L .LT. 0, the coefficients are in reverse order, If L .LT. 0, the coefficients are in reverse order, i.e. i.e. C)**N + ... + TC(N)*(X-C) + TC(N+1) |
| 4 | `A` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | sum of orthogonal polynomials.  PCOEF  changes this fit to its Taylor expansion about any point  C , i.e. writes the polynomial C).  Taking  C=0.  gives the polynomial zero  C  often leads to polynomials which are better scaled and more accurately evaluated. The parameters for  PCOEF  are Work and output array containing values from last call to  POLFIT . |
