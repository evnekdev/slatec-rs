# DSISI

[Family: Dense linear algebra](../families/dense-linear-algebra.md) | [All families](../routines-by-family.md) | [Alphabetical index](../routines-alphabetical.md) | [Coverage](../routine-coverage.md)

## Purpose

Hermitian matrix using the factors obtained from CHIFA.

## Classification

- Historical role: `historical_or_obsolete`
- Program-unit kind: `unknown`
- Identity kind: `historical_catalogue_identity`
- Identity status: `historical_external_dependency`
- Precision: `f64`
- Scalar kind: `real`
- Primary family: `Dense linear algebra`
- Mathematical domain: `dense-linear-algebra`
- Package provenance: `unknown`
- GAMS classifications: `D3D1A`
- Family evidence: `netlib_gams` (`verified`)

## Project coverage

- Source status: `catalogue_only`
- Raw-binding status: `not_bound`
- Build/profile status: `unknown`
- Audit status: `identity_only`
- Safe-API status: `none`
- Implementation status: `not_exposed_as_safe_api`
- Deferment status: Catalogue inclusion does not imply a Rust binding or safe API.

## Providers

- Canonical provider: unavailable

## Official references

- [Netlib TOC](https://www.netlib.org/slatec/toc) — `verified_cached`

## Evidence notes

Description selected from `netlib_slatec_toc` using `TOC Section I/II purpose`; confidence: `high`. External-reference statuses are generated offline from separately cached source files, directory indexes, and TOC evidence.

- Catalogue identity has no reconciled source provider and remains visible for review.

<!-- release-readiness:start -->
## Interface documentation quality

- Evidence level: `purpose_only`
- Description provenance: `unavailable`
- Assessment: only a verified concise purpose is available
- Dedicated family page: [Dense linear algebra](../families/dense-linear-algebra.md)

No independently callable argument list is present in the selected interface inventory for this identity.
<!-- release-readiness:end -->

<!-- raw-api-status:start -->
## Raw Rust API status

This generated status is evidence only; see the [authoritative inventory](../../../generated/raw-api/routine-status.json).

- Public raw API status: `historical-program`
- ABI validation: `pending`
- Canonical Rust path: `not_promoted`
- Public declaration feature: `not_assigned`
- `all`-feature reachability: `not_enabled_by_all`
- Provider-backed callable symbol: `no` (`missing_symbol`)
- Documentation status: `not_documented`
- Compile-test status: `not_compiled`
- Link-test status: `not_tested`
- Runtime validation: `not-recorded`
- Safe-wrapper status: `not_safely_wrapped`
- Exclusion or deferment reason: `catalogue identity has no selected executable provider`
<!-- raw-api-status:end -->
