...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `XT` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | LE. X LXT 1, That is, when multiplicities are present in the break point to the left of X, the largest index is taken for ILEFT. XT is a knot or break point vector of length LXT. |
| 2 | `LXT` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | length of the XT vector. |
| 3 | `X` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | LT. XT(1) 1 -1 if XT(I). LE. X. XT(I+1) then ILEFT=I , MFLAG=0 argument. |
