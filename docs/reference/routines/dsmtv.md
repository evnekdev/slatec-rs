...
- Exact Netlib source: [DSMTV](https://www.netlib.org/slatec/lin/dsmtv.f)
...
| 2 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | IN       Double Precision X(N). The vector that should be multiplied by the transpose of the matrix. |
| 3 | `Y` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | A'*X, where ' denotes transpose. OUT      Double Precision Y(N). The product of the transpose of the matrix and the vector. |
...
Canonical Rust path: `slatec_sys::blas::level1::dsmtv`. Native symbol: `dsmtv_`. Declaration feature: `blas-level1`. Provider feature: `blas-level1`. ABI fingerprint: `unavailable`.
