...
| 2 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision X(N). The vector that should be multiplied by the transpose of the matrix. |
| 3 | `Y` | `output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | A'*X, where ' denotes transpose. Double Precision Y(N). The product of the transpose of the matrix and the vector. |
