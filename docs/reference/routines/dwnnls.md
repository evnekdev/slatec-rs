...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `W` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (MDW, *) | The array W(*,*) is double subscripted with first. |
...
| 7 | `PRGOPT` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | This double precision array is the option vector. If the user is satisfied with the nominal subprogram features set 1 (or PRGOPT(1)=1. 0) Otherwise PRGOPT(*) is a linked list consisting of groups of data of the following form LINK KEY DATA SET The parameters LINK and KEY are each one word. The DATA SET can be comprised of several words. The number of items depends on the value of KEY. The value of LINK points to the first entry of the next group of data within The exception is when there are no more options to change. |
| 8 | `X` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | (WT*F) ( A) ( B), (least squares) subject to components L+1,. ,N nonnegative. The subprogram chooses the heavy weight (or penalty parameter) WT. The parameters for DWNNLS are An array dimensioned at least N, which will contain the N components of the solution vector. |
...
| 12 | `WORK` | `workspace-output` | `workspace` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | A double precision working array of length at least M + 5*N. |
