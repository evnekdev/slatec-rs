...
COMPB computes the roots of the B polynomials using subroutine TEVLS which is a modification the EISPACK program TQLRAT. IERROR is set to 4 if either TEVLS fails or if A(J+1)*C(J) is less than zero for some J. AH,BH are temporary work arrays.
...
- Canonical provider: `main-src/src/compb.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/compb.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/compb.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/compb.f) — `verified_cached`
...
| `IERROR` | unavailable | `INTEGER` (`implicit_rule`) | `*mut crate::FortranInteger` | scalar | IERROR is set to 4 if either TEVLS fails or if A(J+1)*C(J) is less than zero for some J. | IERROR is set to 4 if either TEVLS fails or if A(J+1)*C(J) is less than zero for some J. Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `B` | unavailable | `REAL` (`implicit_rule`) | `*mut f32` | rank 1; dimensions (*) | COMPB computes the roots of the B polynomials using subroutine TEVLS which is a modification the EISPACK program TQLRAT. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
