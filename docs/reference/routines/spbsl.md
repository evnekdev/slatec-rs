...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `ABD` | input/output | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (LDA, *) | On Entry ABD REAL(LDA, N) the output from SPBCO or SPBFA. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `LDA` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | On Entry ABD REAL(LDA, N) the output from SPBCO or SPBFA. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `N` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | On Entry ABD REAL(LDA, N) the output from SPBCO or SPBFA. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
- Compatibility aliases: `slatec_sys::linear_algebra::dense::spbsl`
