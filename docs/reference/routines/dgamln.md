...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Z` | output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | **** A DOUBLE PRECISION ROUTINE **** DGAMLN COMPUTES THE NATURAL LOG OF THE GAMMA FUNCTION FOR Z.GT.0. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `IERR` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | DESCRIPTION OF ARGUMENTS INPUT Z IS D0UBLE PRECISION Z - ARGUMENT, Z.GT.0.0D0 OUTPUT DGAMLN IS DOUBLE PRECISION DGAMLN - NATURAL LOG OF THE GAMMA FUNCTION AT Z.NE.0.0D0 IERR - ERROR FLAG IERR=0, NORMAL RETURN, COMPUTATION COMPLETED IERR=1, Z.LE.0.0D0, NO COMPUTATION | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
