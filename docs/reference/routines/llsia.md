...
| 12 | `NP` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | The first NP columns of A will not be interchanged with other columns even though the pivot strategy would suggest otherwise. 0. WORK()        A real work array dimensioned 5*N.  However, if are not accessed. |
...
| 15 | `RNORM` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Contains the Euclidean length of the NB residual |
| 16 | `W` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Writable real work array for the rank-revealing QR solve. It requires `5*N` elements when `RE` and `AE` are scalar, `4*N` when either is vector-valued, and `3*N` when both are vector-valued. Its leading entries are persistent factorization state for an `INFO=1` continuation call. |
| 17 | `LW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Actual dimension of WORK IWORK, LIW, and the first 2*N locations of WORK as output by the original call to LLSIA. MODE must be equal to the value of MODE in the original call. If MODE.LT.2, only the first N locations of WORK |
