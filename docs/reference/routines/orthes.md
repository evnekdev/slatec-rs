...
| 3 | `LOW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | through IGH to upper Hessenberg form by orthogonal similarity transformations. are two INTEGER variables determined by the balancing subroutine  BALANC.  If  BALANC  has not been 1 and IGH equal to the order of the matrix, N. |
| 4 | `IGH` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | are two INTEGER variables determined by the balancing subroutine  BALANC.  If  BALANC  has not been dimensional REAL array, dimensioned ORT(IGH). Questions and comments should be directed to B. S. Garbow, APPLIED MATHEMATICS DIVISION, ARGONNE NATIONAL LABORATORY |
| 5 | `A` | `input` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (NM, *) | contains the general matrix to be reduced to upper dimensional REAL array, dimensional REAL array, dimensioned A(NM,N). dimensioned A(NM,N). contains the upper Hessenberg matrix.  Some information about the orthogonal transformations used in the reduction is stored in the remaining triangle under the Hessenberg matrix. dimensional REAL array, dimensioned ORT(IGH). Questions and comments should be directed to B. S. Garbow, APPLIED MATHEMATICS DIVISION, ARGONNE NATIONAL LABORATORY |
| 6 | `ORT` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | formations used in the reduction.  Only elements LOW+1 dimensional REAL array, dimensioned ORT(IGH). Questions and comments should be directed to B. S. Garbow, APPLIED MATHEMATICS DIVISION, ARGONNE NATIONAL LABORATORY |
