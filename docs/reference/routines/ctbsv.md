...
- Exact Netlib source: [CTBSV](https://www.netlib.org/slatec/lin/ctbsv.f)
...
| 8 | `X` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (*) | b,   or   A'*x = b,   or   conjg( A')*x = b, is an n by n unit, or non-unit, upper or lower triangular band matrix, with ( k + 1 ) diagonals. No test for singularity or near-singularity is included in this routine. Such tests must be performed before calling this routine. Parameters ========== COMPLEX          array of dimension at least must not be zero. Unchanged on exit. |
...
Canonical Rust path: `slatec_sys::blas::level2::ctbsv`. Native symbol: `ctbsv_`. Declaration feature: `blas-level2`. Provider feature: `blas-level2`. ABI fingerprint: `unavailable`.
