...
| 8 | `KNTOPT` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | knot selection parameter 1 sets knot multiplicity at T(4) and T(N+1) to 4 = 2 sets a symmetric placement of knots about T(4) and T(N+1) = 3 sets T(I)=W(I) and T(N+1+I)=W(3+I),I=1,3 3, then W(1),W(2),W(3) are knot values to the left of X(1) and W(4),W(5),W(6) are knot values to the right of X(NDATA) in increasing order to be supplied by the user Output     T,BCOEF are double precision |
| 9 | `T` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | knot array of length N+4 |
| 10 | `BCOEF` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | B spline coefficient array of length N |
| 11 | `N` | `input-output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | number of coefficients, N=NDATA+2 |
| 12 | `K` | `input-output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | order of spline, K=4 |
| 13 | `W` | `workspace` | `workspace` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (5, *) | 1,6 is supplied by the user work array of dimension at least 5*(NDATA+2) |
...
`W`: 1,6 is supplied by the user work array of dimension at least 5*(NDATA+2)
