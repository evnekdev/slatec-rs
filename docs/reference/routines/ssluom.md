...
- Exact Netlib source: [SSLUOM](https://www.netlib.org/slatec/lin/ssluom.f)
...
| 17 | `RWORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (LENW) | WORK     Real RWORK(LENW). Real array used for workspace.  NL is the number of non- zeros in the lower triangle of the matrix (including the diagonal).  NU is the number of non-zeros in the upper triangle of the matrix (including the diagonal). |
| 18 | `LENW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer. Length of the real workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1) |
