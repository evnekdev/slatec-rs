...
| 7 | `A` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | A(NELT). These arrays should hold the matrix A in either the SLAP Triad format or the SLAP Column format. See "Description", below. If the SLAP Triad format is chosen, it is changed internally to the SLAP Column format. |
...
| 17 | `RWORK` | `workspace-output` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (LENW) | RWORK(LENW). array used for workspace. NL is the number of non- zeros in the lower triangle of the matrix (including the diagonal). NU is the number of non-zeros in the upper. |
| 18 | `LENW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Integer. Length of the real workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1). |
...
`RWORK`: RWORK(LENW). array used for workspace. NL is the number of non- zeros in the lower triangle of the matrix (including the diagonal). NU is the number of non-zeros in the upper.
