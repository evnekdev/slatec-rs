...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `ZR` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | ***A DOUBLE PRECISION ROUTINE*** On KODE=1, ZBESK computes an N member sequence of complex Bessel functions CY(L)=K(FNU+L-1,Z) for real nonnegative orders FNU+L-1, L=1,...,N and complex Z.NE.0 in the cut plane -pi<arg(Z)<=pi where Z=ZR+i*ZI. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
- Compatibility aliases: `slatec_sys::special::numerical::zbesk`
