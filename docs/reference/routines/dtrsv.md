...
- Exact Netlib source: [DTRSV](https://www.netlib.org/slatec/lin/dtrsv.f)
...
| 7 | `X` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | b,   or   A'*x = b, is an n by n unit, or non-unit, upper or lower triangular matrix. No test for singularity or near-singularity is included in this routine. Such tests must be performed before calling this routine. Parameters ========== DOUBLE PRECISION array of dimension at least must not be zero. Unchanged on exit. |
...
Canonical Rust path: `slatec_sys::blas::level2::dtrsv`. Native symbol: `dtrsv_`. Declaration feature: `blas-level2`. Provider feature: `blas-level2`. ABI fingerprint: `unavailable`.
