...
- Safe Rust paths: `slatec::least_squares::least_squares_f32`

## Providers

- Canonical provider: `main-src/src/snls1e.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/snls1e.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/snls1e.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/snls1e.f) — `verified_cached`
...
| 9 | `INFO` | `status-output` | `status` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 1,N) STOP 1000 FORMAT (5X,' FINAL L2 NORM OF THE RESIDUALS',E15.7 // 5X,' EXIT PARAMETER',16X,I10 // 5X,' FINAL APPROXIMATE SOLUTION' // 5X,3E15.7) END SUBROUTINE FCN(IFLAG,M,N,X,FVEC,DUM,IDUM) |
...
| 11 | `WA` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Array argument classified by fixed-form executable read/write analysis. |
...
Canonical Rust path: `slatec_sys::least_squares::snls1e`. Native symbol: `snls1e_`. Declaration feature: `least-squares-nonlinear-easy`. Provider feature: `least-squares-nonlinear-easy`. ABI fingerprint: `unavailable`.
...
- Safe-wrapper status: `slatec::least_squares::least_squares_f32`
