...
On KODE=1, CBESY computes an N member sequence of complex Bessel functions CY(L)=Y(FNU+L-1,Z) for real nonnegative orders FNU+L-1, L=1,...,N and complex Z in the cut plane -pi<arg(Z)<=pi. On KODE=2, CBESY returns the scaled
...
| 3 | `KODE` | `input-output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | A parameter to indicate the scaling option 1 returns 2 (the underflows may not be in an uninterrupted sequence). |
| 4 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Number of terms in the sequence, N>=1. |
| 5 | `CY` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (N) | exp(-abs(Y))*Y(FNU+L-1,Z), L=1,. ,N, Y=Im(Z) which remove the exponential growth in both the upper and lower half planes as Z goes to infinity. Definitions and notation are found in the NBS Handbook of Mathematical Functions (Ref. 1). =2 returns Y(FNU+L-1,Z)*exp(-abs(Y)), L=1,. ,N where Y=Im(Z) Result vector of type COMPLEX. |
| 6 | `NZ` | `status-output` | `status` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Number of underflows set to zero NZ=0 Normal return NZ>0 CY(L)=0 for NZ values of L, usually on. |
| 7 | `CWRK` | `workspace-output` | `workspace` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (N) | A work vector of type COMPLEX and dimension N. |
