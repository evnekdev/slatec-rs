...
| `H` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | RMAX is the maximum ratio by which H can be increased in one step. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `RC` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | Also, RC is reset. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `YH` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 2; dimensions (N, *) | If the caller has changed H, YH must be rescaled. | If the caller has changed H, YH must be rescaled. Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `A` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 2; dimensions (MATDIM, *) | If a failure occurs (in corrector convergence or error test), RMAX is set at RMFAIL for the next increase. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `RH` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | No separable argument description was found in the selected source prologue. | unavailable Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `RMAX` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | RMAX is the maximum ratio by which H can be increased in one step. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
