...
| 14 | `TOL` | `input-output` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | Double Precision. Convergence criterion, as described above. (Reset if IERR=4. ). |
...
| 17 | `ERR` | `output` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | Double Precision. Error estimate of error in final approximate solution, as defined by ITOL. |
...
| 20 | `R` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision R(N). |
| 21 | `Z` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision Z(N). |
| 22 | `P` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision P(N). |
| 23 | `RR` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision RR(N). |
| 24 | `ZZ` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision ZZ(N). |
| 25 | `PP` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision PP(N). |
| 26 | `DZ` | `workspace-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision DZ(N). Double Precision arrays used for workspace. |
| 27 | `RWORK` | `workspace-output` | `workspace` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | and IWORK arrays). The name of the MSOLVE routine must be declared external in the calling program. The calling sequence of MSOLVE is: CALL MSOLVE(N, R, Z, NELT, IA, JA, A, ISYM, RWORK, IWORK) Where N is the number of unknowns, R is the right-hand side vector, and Z is the solution upon return. NELT, IA, JA, A and ISYM define the SLAP matrix data structure: see Description, below. RWORK is a double precision array that can be used to pass necessary preconditioning information and/ or workspace to MSOLVE. IWORK is an integer work array for the same purpose as RWORK. |
