...
- Canonical provider: `lin/ctrmv.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/ctrmv.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/lin/ctrmv.f) — `verified_cached`
...
- Exact Netlib source: [CTRMV](https://www.netlib.org/slatec/lin/ctrmv.f)
...
| 5 | `A` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 2; dimensions (LDA, *) | unit, upper or lower triangular matrix. Parameters ========== is unit triangular as follows: COMPLEX          array of DIMENSION ( LDA, n ). is not referenced. is not referenced. are not referenced either, but are assumed to be unity. Unchanged on exit. |
...
| 7 | `X` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (*) | = A*x,   or   x := A'*x,   or   x := conjg( A')*x, unit, upper or lower triangular matrix. Parameters ========== = A*x. = A'*x. = conjg( A' )*x. Unchanged on exit. COMPLEX          array of dimension at least is overwritten with the is overwritten with the transformed vector x. transformed vector x. must not be zero. Unchanged on exit. |
...
Canonical Rust path: `slatec_sys::blas::level2::ctrmv`. Native symbol: `ctrmv_`. Declaration feature: `blas-level2`. Provider feature: `blas-level2`. ABI fingerprint: `unavailable`.
...
- Canonical Rust path: `slatec_sys::blas::level2::ctrmv`
