...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `A` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | COMPLEX(LDA,N) complex nonsymmetric input matrix. |
...
| 4 | `E` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | COMPLEX(N) on return from CGEEV E contains the eigenvalues of A. See also INFO below. |
| 5 | `V` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | COMPLEX(LDV,N) on return from CGEEV if the user has set JOB = 0 V is not referenced. = nonzero the N eigenvectors of A are stored in the first N columns of V. See also INFO below. (If the input matrix A is nearly degenerate, V will be badly conditioned, i. e. have nearly dependent columns. |
...
| 7 | `WORK` | `workspace-output` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | REAL(3N) temporary storage vector. Contents changed by CGEEV. |
