...
| 2 | `X` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | 1,...,N the sum from K=1 to K=N of 1)*(I-1)*PI/(2*N)) A call of COSQB followed by a call of COSQF will multiply the sequence X by 4*N. Therefore COSQF is the unnormalized inverse of COSQB. WSAVE   contains initialization calculations which must not be destroyed between calls of COSQB or COSQF. |
