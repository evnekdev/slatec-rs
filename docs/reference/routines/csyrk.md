...
- Exact Netlib source: [CSYRK](https://www.netlib.org/slatec/lin/csyrk.f)
...
| 5 | `ALPHA` | `input` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== COMPLEX         . On entry, ALPHA specifies the scalar alpha. Unchanged on exit. |
...
| 8 | `BETA` | `input` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== COMPLEX         . On entry, BETA specifies the scalar beta. Unchanged on exit. |
| 9 | `C` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 2; dimensions (LDC, *) | = alpha*A*A' + beta*C, or = alpha*A'*A + beta*C, is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== = alpha*A*A' + beta*C. = alpha*A'*A + beta*C. Unchanged on exit. must be at least zero. Unchanged on exit. COMPLEX          array of DIMENSION ( LDC, n ). |
...
Canonical Rust path: `slatec_sys::blas::level3::csyrk`. Native symbol: `csyrk_`. Declaration feature: `blas-level3`. Provider feature: `blas-level3`. ABI fingerprint: `unavailable`.
