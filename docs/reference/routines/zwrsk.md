...
ZWRSK COMPUTES THE I BESSEL FUNCTION FOR RE(Z).GE.0.0 BY NORMALIZING THE I FUNCTION RATIOS FROM ZRATI BY THE WRONSKIAN
...
- Canonical provider: `main-src/src/zwrsk.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/zwrsk.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/zwrsk.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/zwrsk.f) — `verified_cached`
...
| `CWR` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (2) | No separable argument description was found in the selected source prologue. | unavailable Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `CWI` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (2) | No separable argument description was found in the selected source prologue. | unavailable Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
