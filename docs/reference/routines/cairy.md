...
| 4 | `AI` | `input-output` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | Ai(z) on ID=0 dAi/dz on ID=1 at z=Z =2 returns exp(zeta)*Ai(z) on ID=0 exp(zeta)*dAi/dz on ID=1 at z=Z where zeta=(2/3)*z**(3/2) Result of type COMPLEX. |
| 5 | `NZ` | `status-output` | `status` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Underflow indicator NZ=0 Normal return NZ=1 AI=0 due to underflow in -pi/3<arg(Z)<pi/3 on KODE=1. |
