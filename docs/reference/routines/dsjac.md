...
- Canonical provider: `lin/dsjac.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/dsjac.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/lin/dsjac.f) — `verified_cached`
...
- Exact Netlib source: [DSJAC](https://www.netlib.org/slatec/lin/dsjac.f)

### Arguments

| # | Argument | Direction | Role | Fortran type | Rust raw type | Shape | Contract |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer. Order of the Matrix. 1       n    n 1       n NELT+1,  where N is  the number of columns in  the matrix and NELT  is the number  of non-zeros in the matrix. Here is an example of the  SLAP Column  storage format for a |
| 2 | `B` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | IN       Double Precision B(N). Right-hand side vector. LX - UX ) AX ) The Sparse Linear Algebra Package (SLAP) utilizes two matrix data structures: 1) the  SLAP Triad  format or  2)  the SLAP Column format.  The user can hand this routine either of the of these data structures and SLAP  will figure out which one is being used and act accordingly. =================== S L A P Triad format =================== This routine requires that the  matrix A be   stored in  the SLAP  Triad format.  In  this format only the non-zeros  are stored.  They may appear in  *ANY* order.  The user supplies |
...
| 10 | `TOL` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | inv is the inverse of the diagonal of A. INOUT    Double Precision. |
...
| 16 | `RWORK` | `workspace` | `workspace` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (LENW) | WORK     Double Precision RWORK(LENW). Double Precision array used for workspace. 4*N. Upon return the following locations of IWORK hold information which may be of use to the user: |
| 17 | `LENW` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer. 4*N. |
| 18 | `IWORK` | `workspace` | `workspace` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (LENIW) | WORK     Integer IWORK(LENIW). Used to hold pointers into the double precision workspace, Amount of Integer workspace actually used. Amount of Double Precision workspace actually used. 10. |
