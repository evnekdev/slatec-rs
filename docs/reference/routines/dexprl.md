...
- Exact Netlib source: [DEXPRL](https://www.netlib.org/slatec/fnlib/dexprl.f)

### Arguments

| # | Argument | Direction | Role | Fortran type | Rust raw type | Shape | Contract |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `X` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | 1)/X. 1.0) / X.   For small ABS(X) the 1.0) / X.   For small ABS(X) the Taylor series is used.  If X is negative the reflection formula Taylor series is used.  If X is negative the reflection formula EXP(X) * EXPREL(ABS(X)) may be used.  This reflection formula will be of use when the evaluation for small ABS(X) is done by Chebyshev series rather than Taylor series. |
...
Canonical Rust path: `slatec_sys::special::elementary::dexprl`. Native symbol: `dexprl_`. Declaration feature: `special-elementary`. Provider feature: `special-elementary`. ABI fingerprint: `unavailable`.
