...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `Z` | `input` | `scalar` | `COMPLEX` | `*mut crate::Complex32` | scalar | Nonzero argument of type COMPLEX 2 and Im(Z)<0, then |
| 2 | `FNU` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Initial order of type REAL, FNU>=0 1,Z), L=1,...,N =2  returns 1,Z)*exp(-(3-2M)*Z*i), L=1,...,N |
| 3 | `KODE` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | A parameter to indicate the scaling option 1  returns |
| 4 | `M` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 1,Z), L=1,...,N =2  returns 1,Z)*exp(-(3-2M)*Z*i), L=1,...,N Superscript of Hankel function, M=1 or 2 2 and Im(Z)<0, then |
| 5 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Number of terms in the sequence, N>=1 |
| 6 | `CY` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (N) | 1,Z), L=1,...,N =2  returns 1,Z)*exp(-(3-2M)*Z*i), L=1,...,N Result vector of type COMPLEX 0 for NZ values of L (if M=1 and plementary half planes, the underflows may not be in an uninterrupted sequence) |
