...
DPPERM rearranges the data vector DX according to the permutation IPERM: DX(I) <--- DX(IPERM(I)). IPERM could come from one of the sorting routines IPSORT, SPSORT, DPSORT or HPSORT.
...
- Canonical provider: `main-src/src/dpperm.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/dpperm.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/dpperm.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/dpperm.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `DX` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (*) | DPPERM rearranges the data vector DX according to the permutation IPERM: DX(I) <--- DX(IPERM(I)). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `IPERM` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | rank 1; dimensions (*) | DPPERM rearranges the data vector DX according to the permutation IPERM: DX(I) <--- DX(IPERM(I)). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
