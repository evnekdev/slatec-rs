...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | DOUBLE PRECISION X INTEGER IX IF RADIX**(-2L) .LE. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `IX` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | DOUBLE PRECISION X INTEGER IX IF RADIX**(-2L) .LE. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
