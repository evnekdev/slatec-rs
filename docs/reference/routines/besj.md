...
| 4 | `Y` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | a vector whose first N components contain values for J/sub(ALPHA+K-1)/(X), K=1,. ,N 0. 0E0, K=N-NZ+1,. ,N. |
...
Canonical Rust path: `slatec_sys::special::bessel::besj`. Native symbol: `besj_`. Declaration feature: `special`. Provider feature: `special-real`. ABI fingerprint: `subroutine:void(mut_f32,mut_f32,mut_i32,mut_f32_ptr_rank1,mut_i32)`.
