...
- Canonical provider: `lin/stbmv.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/stbmv.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/lin/stbmv.f) — `verified_cached`
...
- Exact Netlib source: [STBMV](https://www.netlib.org/slatec/lin/stbmv.f)
...
| 8 | `X` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | = A*x, or x := A'*x, where x is an n element vector and A is an n by n unit, or non-unit, upper or lower triangular band matrix, with ( k + 1) diagonals. = A*x. REAL array of dimension at least ( 1 + ( n - 1 )*abs( INCX ) ). Before entry, the incremented array X must contain the n element vector x. On exit, X is overwritten with the transformed vector x. |
...
Canonical Rust path: `slatec_sys::blas::level2::stbmv`. Native symbol: `stbmv_`. Declaration feature: `blas-level2`. Provider feature: `blas-level2`. ABI fingerprint: `unavailable`.
...
- Canonical Rust path: `slatec_sys::blas::level2::stbmv`
