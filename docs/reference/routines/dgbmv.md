...
| 9 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | is an DOUBLE PRECISION array of DIMENSION at least must not be zero. Unchanged on exit. |
...
| 11 | `BETA` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | is an DOUBLE PRECISION. On entry, BETA specifies the scalar beta. When BETA is supplied as zero then Y need not be set on input. Unchanged on exit. |
| 12 | `Y` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | = alpha*A*x + beta*y,   or   y := alpha*A'*x + beta*y, is an = alpha*A*x + beta*y. = alpha*A'*x + beta*y. = alpha*A'*x + beta*y. Unchanged on exit. DOUBLE PRECISION array of DIMENSION at least is overwritten by the updated vector y. is overwritten by the updated vector y. must not be zero. Unchanged on exit. |
