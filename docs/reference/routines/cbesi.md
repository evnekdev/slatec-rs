...
| 2 | `FNU` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Initial order of type REAL, FNU>=0 1,Z), L=1,...,N =2  returns |
...
| 5 | `CY` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (N) | 1,Z), L=1,...,N =2  returns abs(X))*I(FNU+L-1,Z), L=1,...,N where X=Re(Z) Result vector of type COMPLEX NZ+1,...,N |
