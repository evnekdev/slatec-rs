...
- Canonical provider: `main-src/src/ivout.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/ivout.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/ivout.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/ivout.f) — `verified_cached`
...
| `IX` | unavailable | `UNKNOWN` (`unknown`) | `UNKNOWN` | rank 1; dimensions (*) | No separable argument description was found in the selected source prologue. | unavailable Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
