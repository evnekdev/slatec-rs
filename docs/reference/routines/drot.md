...
| 2 | `DX` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | double precision vector with N elements rotated vector DX (unchanged if N .LE. 0) 1, where LX = 1 if INCX .GE. 0, else LX = 1+(1-N)*INCX, and similarly for DY using LY and INCY. |
...
| 4 | `DY` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | double precision vector with N elements rotated vector DY (unchanged if N .LE. 0) Multiply the 2 x 2 matrix  ( DC DS) times the 2 x N matrix (DX**T) |
...
| 7 | `DS` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | D.P. element of rotation matrix DC)                        (DY**T) where **T indicates transpose.  The elements of DX are in |
