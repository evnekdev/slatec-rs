...
| 3 | `ALPHA` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | are scalars, x and y are n element vectors and A is an n by n symmetric matrix, supplied in packed form. Parameters ========== DOUBLE PRECISION. On entry, ALPHA specifies the scalar alpha. Unchanged on exit. |
| 4 | `AP` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | DOUBLE PRECISION array of DIMENSION at least ( ( n*( n + 1))/2). contain a( 1, 2 ) contain a( 1, 2 ) and a( 2, 2 ) respectively, and so on. and a( 2, 2 ) respectively, and so on. contain a( 2, 1 ) contain a( 2, 1 ) and a( 3, 1 ) respectively, and so on. and a( 3, 1 ) respectively, and so on. Unchanged on exit. Unchanged on exit. |
| 5 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | DOUBLE PRECISION array of dimension at least must not be zero. Unchanged on exit. |
...
| 7 | `BETA` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | are scalars, x and y are n element vectors and A is an n by n symmetric matrix, supplied in packed form. Parameters ========== DOUBLE PRECISION. On entry, BETA specifies the scalar beta. When BETA is supplied as zero then Y need not be set on input. Unchanged on exit. |
| 8 | `Y` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | = alpha*A*x + beta*y, DOUBLE PRECISION array of dimension at least is overwritten by the updated is overwritten by the updated vector y. vector y. must not be zero. Unchanged on exit. |
