...
PSI(X) calculates the psi (or digamma) function for real argument X. PSI(X) is the logarithmic derivative of the gamma function of X. Series for PSI on the interval 0. to 1.00000D+00
...
- Exact Netlib source: [PSI](https://www.netlib.org/slatec/fnlib/psi.f)
...
significant figures required  16.39 decimal places required  17.37 Series for APSI       on the interval  0.          to  2.50000D-01 significant figures required  14.42 decimal places required  16.86
...
Canonical Rust path: `slatec_sys::special::gamma::psi`. Native symbol: `psi_`. Declaration feature: `special-gamma`. Provider feature: `special-gamma`. ABI fingerprint: `unavailable`.
