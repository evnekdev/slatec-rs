...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `A` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 2; dimensions (LDA, *) | B , follow CHIFA by CHISL. To compute  INVERSE(A)*C , follow CHIFA by CHISL. To compute  DETERMINANT(A) , follow CHIFA by CHIDI. To compute  INERTIA(A) , follow CHIFA by CHIDI. To compute  INVERSE(A) , follow CHIFA by CHIDI. On Entry COMPLEX(LDA,N) the Hermitian matrix to be factored. Only the diagonal and upper triangle are used. a block diagonal matrix and the multipliers which were used to obtain it. U*D*CTRANS(U) where  U  is a product of permutation and unit upper triangular matrices , CTRANS(U) is the conjugate transpose of  U , and  D  is block diagonal with 1 by 1 and 2 by 2 blocks. |
