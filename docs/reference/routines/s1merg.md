...
This subroutine merges two ascending strings of numbers in the array TCOS. The first string is of length M1 and starts at TCOS(I1+1). The second string is of length M2 and starts at TCOS(I2+1). The merged string goes into TCOS(I3+1).
...
- Canonical provider: `main-src/src/s1merg.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/s1merg.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/s1merg.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/s1merg.f) — `verified_cached`
