...
| 5 | `CY` | `input` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (N) | 1,Z) for real nonnegative 1,Z),  L=1,...,N which remove the exponential growth in both the left and right half planes as Z goes to infinity.  Definitions and notation are found in the NBS Handbook of Mathematical Functions (Ref. 1). 1,Z), L=1,...,N =2  returns 1,Z)*EXP(Z), L=1,...,N Result vector of type COMPLEX 0 for NZ values of L (if Re(Z)>0 0 for L=1,...,NZ; in the complementary half plane the underflows may not be in an uninterrupted sequence) |
