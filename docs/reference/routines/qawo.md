...
Computation of oscillatory integrals Standard fortran subroutine Real version PARAMETERS ON ENTRY
...
- Safe Rust paths: `slatec::quadrature::integrate_oscillatory_f32`

## Providers

- Canonical provider: `main-src/src/qawo.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/qawo.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/qawo.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/qawo.f) — `verified_cached`
...
- Exact Netlib source: [QAWO](https://www.netlib.org/slatec/src/qawo.f)

### Arguments

| # | Argument | Direction | Role | Fortran type | Rust raw type | Shape | Contract |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `F` | `callback` | `callback` | `REAL` | `reviewed unsafe extern callback function pointer` | scalar | Real Function subprogram defining the function The actual name for F needs to be declared E X T E R N A L in the driver program. |
| 2 | `A` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Real Lower limit of integration |
| 3 | `B` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Real Upper limit of integration DIMENSIONING PARAMETERS A)*2**(-L), A)*2**(1-L). |
| 4 | `OMEGA` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Real Parameter in the integrand weight function |
...
| 7 | `EPSREL` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Real Relative accuracy requested If EPSABS.LE.0 and 28), 28)) or (INTEGR.NE.1 AND INTEGR.NE.2), or LENIW.LT.2 OR MAXP1.LT.1 or |
| 8 | `RESULT` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | Real Approximation to the integral are set to zero. Except when LENIW, MAXP1 or LENW are invalid, WORK(LIMIT*2+1), WORK(LIMIT*3+1), |
| 9 | `ABSERR` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | Real Estimate of the modulus of the absolute error, which should equal or exceed ABS(I-RESULT) are set to zero. Except when LENIW, MAXP1 or LENW are invalid, WORK(LIMIT*2+1), WORK(LIMIT*3+1), |
...
| 17 | `WORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | is set to A and WORK(LIMIT+1) to ARRAYS form a decreasing sequence, with LIMIT = LENW/2 , and K = LAST Real Vector of dimension at least LENW On return contain the left contain the left end points of the subintervals in the end points of the subintervals in the partition of (A,B), partition of (A,B), ..., WORK(LIMIT+LAST) contain the right end points, ..., WORK(LIMIT*2+LAST) contain the integral approximations over the subintervals, ..., WORK(LIMIT*3+LAST) contain the error estimates. ..., WORK(LIMIT*4+MAXP1*25) Provide space for storing the Chebyshev moments. Note that LIMIT = LENW/2. |
...
`WORK`: is set to A and WORK(LIMIT+1) to ARRAYS form a decreasing sequence, with LIMIT = LENW/2 , and K = LAST Real Vector of dimension at least LENW On return contain the left contain the left end points of the subintervals in the end points of the subintervals in the partition of (A,B), partition of (A,B), ..., WORK(LIMIT+LAST) contain the right end points, ..., WORK(LIMIT*2+LAST) contain the integral approximations over the subintervals, ..., WORK(LIMIT*3+LAST) contain the error estimates. ..., WORK(LIMIT*4+MAXP1*25) Provide space for storing the Chebyshev moments. Note that LIMIT = LENW/2.

### Provider, ABI, and safety

Canonical Rust path: `slatec_sys::quadrature::qawo`. Native symbol: `qawo_`. Declaration feature: `quadrature-oscillatory`. Provider feature: `quadrature-oscillatory`. ABI fingerprint: `unavailable`.
...
- Canonical Rust path: `slatec_sys::quadrature::qawo`
...
- Safe-wrapper status: `slatec::quadrature::integrate_oscillatory_f32`
