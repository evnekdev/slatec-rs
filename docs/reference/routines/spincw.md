...
- Canonical provider: `main-src/src/spincw.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/spincw.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/spincw.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/spincw.f) — `verified_cached`
...
| `COSTS` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | IT IMPLEMENTS THE PROCEDURE (INITIALIZE REDUCED COSTS AND STEEPEST EDGE WEIGHTS). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
