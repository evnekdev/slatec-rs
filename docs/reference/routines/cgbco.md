...
- Exact Netlib source: [CGBCO](https://www.netlib.org/slatec/lin/cgbco.f)
...
| 8 | `Z` | `output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (*) | COMPLEX(N) a work vector whose contents are usually unimportant. If A is close to a singular matrix, then Z is an approximate null vector in the sense that NORM(A*Z) = RCOND*NORM(A)*NORM(Z). Band Storage if A is a band matrix, the following program segment will set up the input. |
