...
- Safe Rust paths: `slatec::special::elementary::sin_degrees`

## Providers

- Canonical provider: `fnlib/dsindg.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/fnlib/dsindg.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | DSINDG(X) calculates the double precision sine for double precision argument X where X is in degrees. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
Canonical path: `slatec_sys::special::elementary::dsindg`. Native symbol: `dsindg_`. Feature: `special-elementary`. Provider status: `selected_provider_verified`. ABI fingerprint: `unavailable`. Every pointer must be aligned and valid for the full source-defined readable or writable extent; callers must uphold array dimensions, leading dimensions, workspace formulas, aliasing restrictions, callback lifetimes, and process-global runtime serialization.
...
- Compatibility aliases: `slatec_sys::families::special_elementary::dsindg`
...
- Safe-wrapper status: `slatec::special::elementary::sin_degrees`
