...
| 4 | `RESULT` | `output` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | Approximation to the integral I is computed by applying the 31-POINT GAUSS-KRONROD RULE (RESK), obtained by optimal addition of abscissae to the 15-POINT GAUSS RULE (RESG). |
| 5 | `ABSERR` | `output` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | Estimate of the modulus of the modulus, which should not exceed ABS(I-RESULT). |
