...
- Canonical provider: `lin/dsli2.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/dsli2.f) — `verified_cached`
...
- Exact Netlib source: [DSLI2](https://www.netlib.org/slatec/lin/dsli2.f)
...
| 3 | `X` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | OUT      Double Precision X(N). Solution to Lx = b. |
| 4 | `NEL` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer. Number of non-zeros in the EL array. |
| 5 | `IEL` | `input` | `array` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (NEL) | IN       Integer IEL(NEL). contain the unit lower triangular factor   of the incomplete decomposition   of the A  matrix  stored in SLAP Row format.  The diagonal of  ones *IS* stored.  This structure can be  set up by the  DS2LT  routine.  See  the "Description", below, for more details about the  SLAP Row format. |
| 6 | `JEL` | `input` | `array` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (NEL) | IN       Integer JEL(NEL). contain the unit lower triangular factor   of the incomplete decomposition   of the A  matrix  stored in SLAP Row format.  The diagonal of  ones *IS* stored.  This structure can be  set up by the  DS2LT  routine.  See  the "Description", below, for more details about the  SLAP Row format. |
| 7 | `EL` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (NEL) | IN       Double Precision EL(NEL). contain the unit lower triangular factor   of the incomplete decomposition   of the A  matrix  stored in SLAP Row format.  The diagonal of  ones *IS* stored.  This structure can be  set up by the  DS2LT  routine.  See  the "Description", below, for more details about the  SLAP Row format. |
...
Canonical Rust path: `slatec_sys::linear_algebra::dense::dsli2`. Native symbol: `dsli2_`. Declaration feature: `linear-algebra`. Provider feature: `linear-algebra-real`. ABI fingerprint: `subroutine:void(mut_i32,mut_f64_ptr_rank1,mut_f64_ptr_rank1,mut_i32,mut_i32_ptr_rank1,mut_i32_ptr_rank1,mut_f64_ptr_rank1)`.
