...
- Canonical provider: `lin/sgtsl.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/sgtsl.f) — `verified_cached`
...
| `C` | input | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | C REAL(N) is the subdiagonal of the tridiagonal matrix. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `D` | input | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | D REAL(N) is the diagonal of the tridiagonal matrix. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `E` | input | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | E REAL(N) is the superdiagonal of the tridiagonal matrix. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `B` | input/output | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | B REAL(N) is the right hand side vector. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
- Compatibility aliases: `slatec_sys::linear_algebra::dense::sgtsl`
