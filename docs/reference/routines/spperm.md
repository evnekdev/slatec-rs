...
SPPERM rearranges the data vector X according to the permutation IPERM: X(I) <--- X(IPERM(I)). IPERM could come from one of the sorting routines IPSORT, SPSORT, DPSORT or HPSORT.
...
- Canonical provider: `main-src/src/spperm.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/spperm.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/spperm.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/spperm.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | SPPERM rearranges the data vector X according to the permutation IPERM: X(I) <--- X(IPERM(I)). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `IPERM` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | rank 1; dimensions (*) | SPPERM rearranges the data vector X according to the permutation IPERM: X(I) <--- X(IPERM(I)). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
