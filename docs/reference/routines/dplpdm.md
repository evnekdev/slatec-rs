...
THIS SUBPROGRAM IS FROM THE DSPLP( ) PACKAGE. IT PERFORMS THE TASK OF DEFINING THE ENTRIES OF THE BASIS MATRIX AND DECOMPOSING IT USING THE LA05 PACKAGE. IT IS THE MAIN PART OF THE PROCEDURE (DECOMPOSE BASIS MATRIX).
...
- Canonical provider: `main-src/src/dplpdm.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/dplpdm.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/dplpdm.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/dplpdm.f) — `verified_cached`
