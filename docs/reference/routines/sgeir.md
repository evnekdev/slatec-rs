...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `A` | input/output | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (LDA, *) | Subroutine SGEIR solves a general NxN system of single precision linear equations using LINPACK subroutines SGEFA and SGESL. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `N` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | That is, if A is an NxN real matrix and if X and B are real N-vectors, then SGEIR solves the equation A*X=B. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
