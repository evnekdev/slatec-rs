...
| 2 | `X` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | IN       Real X(N). The vector that should be multiplied by the transpose of the matrix. |
| 3 | `Y` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | A'*X, where ' denotes transpose. OUT      Real Y(N). The product of the transpose of the matrix and the vector. |
