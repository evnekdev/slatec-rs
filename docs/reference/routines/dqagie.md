...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `F` | `callback` | `callback` | `DOUBLE PRECISION` | `reviewed unsafe extern callback function pointer` | scalar | INFINITY,BOUND) INFINITY,+INFINITY), hopefully satisfying following claim for accuracy Double precision Function subprogram defining the integrand function F(X). The actual name for F needs to be declared E X T E R N A L in the driver program. |
| 2 | `BOUND` | `input` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | Double precision Finite bound of integration range (has no meaning if interval is doubly-infinite) |
| 3 | `INF` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Double precision Indicating the kind of integration range involved 1 corresponds to  (BOUND,+INFINITY), 1            to  (-INFINITY,BOUND), INFINITY,+INFINITY). |
...
| 11 | `ALIST` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are set to 0 and 1 respectively. Double precision Vector of dimension at least LIMIT, the first |
| 12 | `BLIST` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are set to 0 and 1 respectively. Double precision Vector of dimension at least LIMIT, the first |
| 13 | `RLIST` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | Double precision Vector of dimension at least LIMIT, the first |
| 14 | `ELIST` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | are set to zero. Double precision Vector of dimension at least LIMIT,  the first |
