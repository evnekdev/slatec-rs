...
- Exact Netlib source: [SIR](https://www.netlib.org/slatec/lin/sir.f)

### Arguments

| # | Argument | Direction | Role | Fortran type | Rust raw type | Shape | Contract |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | IN       Integer. Order of the Matrix. hand side vector and Z is the solution upon return.  NELT, IA, JA, A and 1       n 1       n is the number of columns in the matrix and |
| 2 | `B` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | IN       Real B(N). Right-hand side vector. AX  ). -1   -1 If M =  A then this  is the  standard  iterative  refinement algorithm and the "subtraction" in the  residual calculation should be done in double precision (which it is  not in this routine). If M = DIAG(A), the diagonal of A, then iterative refinement is  known  as  Jacobi's  method.   The  SLAP  routine  SSJAC implements this iterative strategy. If M = L, the lower triangle of A, then iterative refinement is known as Gauss-Seidel.   The SLAP routine SSGS implements this iterative strategy. This routine does  not care  what matrix data   structure is used for  A and M.  It simply   calls  the MATVEC and MSOLVE routines, with  the arguments as  described above.  The user could write any type of structure and the appropriate MATVEC and MSOLVE routines.  It is assumed  that A is stored in the |
| 3 | `X` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | INOUT    Real X(N). On input X is your initial guess for solution vector. On output X is the final approximate solution. AX  ). AX  ). -1   -1 -1   -1 If M =  A then this  is the  standard  iterative  refinement If M =  A then this  is the  standard  iterative  refinement algorithm and the "subtraction" in the  residual calculation algorithm and the "subtraction" in the  residual calculation should be done in double precision (which it is  not in this should be done in double precision (which it is  not in this routine). routine). If M = DIAG(A), the diagonal of A, then iterative refinement If M = DIAG(A), the diagonal of A, then iterative refinement is  known  as  Jacobi's  method.   The  SLAP  routine  SSJAC is  known  as  Jacobi's  method.   The  SLAP  routine  SSJAC implements this iterative strategy. implements this iterative strategy. If M = L, the lower triangle of A, then iterative refinement If M = L, the lower triangle of A, then iterative refinement is known as Gauss-Seidel.   The SLAP routine SSGS implements is known as Gauss-Seidel.   The SLAP routine SSGS implements this iterative strategy. this iterative strategy. This routine does  not care  what matrix data   structure is This routine does  not care  what matrix data   structure is used for  A and M.  It simply   calls  the MATVEC and MSOLVE used for  A and M.  It simply   calls  the MATVEC and MSOLVE routines, with  the arguments as  described above.  The user routines, with  the arguments as  described above.  The user could write any type of structure and the appropriate MATVEC could write any type of structure and the appropriate MATVEC and MSOLVE routines.  It is assumed  that A is stored in the and MSOLVE routines.  It is assumed  that A is stored in the |
...
| 12 | `TOL` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | inv is the inverse of the diagonal of A. INOUT    Real. |
...
| 15 | `ERR` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | OUT      Real. |
...
| 18 | `R` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | is supplied via hand side vector and Z is the solution upon return.  NELT, IA, JA, A and WORK     Real R(N). |
| 19 | `Z` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | is supplied via WORK     Real Z(N). |
| 20 | `DZ` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | WORK     Real DZ(N). Real arrays used for workspace. |
| 21 | `RWORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | and IWORK arrays).  The name of the MSOLVE routine must be declared external in the calling program.  The calling is a real array that can be used to pass necessary preconditioning information and/or WORK     Real RWORK(USER DEFINED). Real array that can be used by  MSOLVE. |
| 22 | `IWORK` | `workspace` | `workspace` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (*) | is an integer work array for the same purpose as RWORK. WORK     Integer IWORK(USER DEFINED). Integer array that can be used by  MSOLVE. |
