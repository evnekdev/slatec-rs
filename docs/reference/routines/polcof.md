...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `XX` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | is of the form The point about which the Taylor expansion is to be made. |
| 2 | `N` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | XX)**(N-1)). Between the call to POLINT and the call to POLCOF the variable N **** must remain unchanged between the |
| 3 | `X` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | must not be altered. must remain unchanged between the *     call to POLINT or the call to POLCOF. |
| 4 | `C` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | must not be altered. must remain unchanged between the **** OUTPUT PARAMETER |
| 5 | `D` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | is of the form XX) +D(3)*((Z-XX)**2) + ... + XX) +D(3)*((Z-XX)**2) + ... + XX)**(N-1)). Between the call to POLINT and the call to POLCOF the variable N The array of coefficients for the Taylor expansion as explained in the abstract STORAGE PARAMETER |
