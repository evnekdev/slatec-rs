...
| 2 | `X` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | the array of abscissas (all of which must be distinct). |
| 3 | `Y` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | the array of ordinates. |
| 4 | `C` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | an array of information used by subroutines Dimensioning Information ******* Arrays X,Y, and C must be dimensioned at least N in the calling program. |
