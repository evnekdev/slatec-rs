...
| `N` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | On Entry AP COMPLEX(N*(N+1)/2) the output from CSPFA. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
- Compatibility aliases: `slatec_sys::linear_algebra::dense::complex::cspsl`
