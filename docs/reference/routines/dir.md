...
| 20 | `DZ` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | WORK     Double Precision DZ(N). Double Precision arrays used for workspace. |
| 21 | `RWORK` | `workspace` | `workspace` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | and IWORK arrays).  The name of the MSOLVE routine must be declared external in the calling program.  The calling is a double precision array that can be used to pass necessary preconditioning information WORK     Double Precision RWORK(USER DEFINED). Double Precision array that can be used by  MSOLVE. |
