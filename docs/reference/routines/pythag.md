...
- Canonical provider: `main-src/src/pythag.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/pythag.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/pythag.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `A` | unavailable | `REAL` (`explicit`) | `*mut f32` | scalar | Finds sqrt(A**2+B**2) without overflow or destructive underflow | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `B` | unavailable | `REAL` (`explicit`) | `*mut f32` | scalar | Finds sqrt(A**2+B**2) without overflow or destructive underflow | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
