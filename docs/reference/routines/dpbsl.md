...
A division by zero will occur if the input factor contains a zero on the diagonal.  Technically this indicates singularity, but it is usually caused by improper subroutine arguments.  It will not occur if the subroutines are called correctly, and  INFO .EQ. 0 . To compute  INVERSE(A) * C  where  C  is a matrix with  P  columns CALL DPBCO(ABD,LDA,N,RCOND,Z,INFO) IF (RCOND is too small .OR. INFO .NE. 0) GO TO ... DO 10 J = 1, P CALL DPBSL(ABD,LDA,N,C(1,J)) 10 CONTINUE
