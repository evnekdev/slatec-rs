...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `DNU1` | input | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | The input values to DXLEGF are DNU1, NUDIFF, MU1, MU2, THETA, and ID. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `NUDIFF` | input | `INTEGER` (`implicit_rule`) | `*mut crate::FortranInteger` | scalar | The input values to DXLEGF are DNU1, NUDIFF, MU1, MU2, THETA, and ID. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `THETA` | input | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | PN(MU,NU1,X), PN(MU,NU1+1,X), ..., PN(MU,NU2,X) where X = COS(THETA). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `ID` | unavailable | `INTEGER` (`implicit_rule`) | `*mut crate::FortranInteger` | scalar | The input values to DXLEGF are DNU1, NUDIFF, MU1, MU2, THETA, and ID. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `PQA` | unavailable | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (*) | In each case the vector of computed Legendre function values is returned in the extended-range vector (PQA(I),IPQA(I)). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
- Compatibility aliases: `slatec_sys::special::numerical::dxlegf`
