...
| 5 | `DET` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (2) | Array argument classified by fixed-form executable read/write analysis. |
| 6 | `WORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Workspace argument classified by fixed-form executable read/write analysis. |
...
A division by zero will occur if the input factor contains a zero on the diagonal and the inverse is requested. It will not occur if the subroutines are called correctly and if SGECO has set RCOND .GT. 0.0 or SGEFA has set INFO .EQ. 0 .
