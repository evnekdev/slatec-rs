...
| `X` | unavailable | `REAL` (`implicit_rule`) | `*mut f32` | rank 1; dimensions (*) | PRODP applies a sequence of matrix operations to the vector X and stores the result in Y (periodic boundary conditions). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `Y` | unavailable | `REAL` (`implicit_rule`) | `*mut f32` | rank 1; dimensions (*) | PRODP applies a sequence of matrix operations to the vector X and stores the result in Y (periodic boundary conditions). | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
