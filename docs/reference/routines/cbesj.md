...
| 5 | `CY` | `input-output` | `array` | `COMPLEX` | `*mut crate::Complex32` | rank 1; dimensions (N) | exp(-abs(Y))*J(FNU+L-1,Z), L=1,. ,N and Y=Im(Z) which remove the exponential growth in both the upper and lower half planes as Z goes to infinity. Definitions and notation are found in the NBS Handbook of Mathematical Functions (Ref. 1). =2 returns J(FNU+L-1,Z)*exp(-abs(Y)), L=1,. ,N where Y=Im(Z) Result vector of type COMPLEX. |
