...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `A` | `input-output` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (MDA, *) | Linear coefficient matrix of AX=B, with MDA the Contains the triangular part of the reduced matrix and the transformation information. It together with the first 2*MIN(M,N) elements of WORK (see below) completely specify the factorization of A. |
...
| 5 | `B` | `input-output` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (MDB, *) | Right hand side(s), with MDB the actual first Contains the N by NB solution matrix X. |
...
| 8 | `RNORM` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Vector of length at least NB. On input the contents of RNORM are unused. Contains the Euclidean length of the NB residual vectors B(I)-AX(I), I=1,NB. |
| 9 | `WORK` | `workspace-output` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | A real work array dimensioned 5*MIN(M,N). The first 2*MIN(M,N) locations of WORK contain value necessary to reproduce the factorization of A. |
