...
SPELI4 sets up vectors and arrays for input to BLKTRI and computes a second order solution in USOL. A return jump to SEPX4 occurs if IORDER=2. If IORDER=4 a fourth order solution is generated in USOL.
...
- Canonical provider: `main-src/src/speli4.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/speli4.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/speli4.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/speli4.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `IORDER` | unavailable | `INTEGER` (`implicit_rule`) | `*mut crate::FortranInteger` | scalar | A return jump to SEPX4 occurs if IORDER=2. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `A` | input | `REAL` (`implicit_rule`) | `*mut f32` | scalar | SPELI4 sets up vectors and arrays for input to BLKTRI and computes a second order solution in USOL. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `USOL` | unavailable | `REAL` (`implicit_rule`) | `*mut f32` | rank 2; dimensions (IDMN, *) | SPELI4 sets up vectors and arrays for input to BLKTRI and computes a second order solution in USOL. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
