...
VNWRMS computes a weighted root-mean-square vector norm for the integrator package DEBDF.
...
- Canonical provider: `main-src/src/vnwrms.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/vnwrms.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/vnwrms.f) — `verified_cached`
...
| `V` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | No separable argument description was found in the selected source prologue. | unavailable Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
