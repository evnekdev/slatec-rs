...
| 2 | `B` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Readable right-hand-side vector of length `N` for the SLAP lower-triangular preconditioner solve. The computed vector is returned through `X`, not by retaining `B`. |
...
| 10 | `IWORK` | `workspace-output` | `workspace` | `INTEGER` | `*mut crate::FortranInteger` | rank 1; dimensions (*) | NEL Starting location of IEL in IWORK. Starting location of JEL in IWORK. Starting location of EL in RWORK. Starting location of DINV in RWORK. See the DESCRIPTION of SLLTI2 for details. |
