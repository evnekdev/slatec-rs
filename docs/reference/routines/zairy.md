...
- Exact Netlib source: [ZAIRY](https://www.netlib.org/slatec/src/zairy.f)
...
| 3 | `ID` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 0 or ID=1 respectively. On Order of derivative, ID=0 or ID=1 |
| 4 | `KODE` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | 1, ZAIRY computes the complex Airy function Ai(z) 2, a scaling option exp(zeta)*Ai(z) or exp(zeta)*dAi/dz is provided to remove the exponential decay in -pi/3<arg(z) <pi/3 and the exponential growth in pi/3<abs(arg(z))<pi where zeta=(2/3)*z**(3/2). While the Airy functions Ai(z) and dAi/dz are analytic in the whole z-plane, the corresponding scaled functions defined 2 have a cut along the negative real axis. A parameter to indicate the scaling option 1  returns AI=Ai(z)  on ID=0 AI=dAi/dz on ID=1 at z=Z =2  returns AI=exp(zeta)*Ai(z)  on ID=0 AI=exp(zeta)*dAi/dz on ID=1 at z=Z where zeta=(2/3)*z**(3/2) 1) |
| 5 | `AIR` | `output` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | DOUBLE PRECISION real part of result |
| 6 | `AII` | `output` | `scalar` | `DOUBLE PRECISION` | `*mut f64` | scalar | DOUBLE PRECISION imag part of result |
| 7 | `NZ` | `status-output` | `status` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Underflow indicator 0    Normal return 1    AI=0 due to underflow in -pi/3<arg(Z)<pi/3 on KODE=1 |
...
Canonical Rust path: `slatec_sys::special::zairy`. Native symbol: `zairy_`. Declaration feature: `special`. Provider feature: `special-real`. ABI fingerprint: `subroutine:void(mut_f64,mut_f64,mut_i32,mut_i32,mut_f64,mut_f64,mut_i32,mut_i32)`.
