...
| 4 | `KODE` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | A parameter to indicate the scaling option 1  returns AI=Ai(z)  on ID=0 AI=dAi/dz on ID=1 at z=Z =2  returns AI=exp(zeta)*Ai(z)  on ID=0 AI=exp(zeta)*dAi/dz on ID=1 at z=Z where zeta=(2/3)*z**(3/2) |
