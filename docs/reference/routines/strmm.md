...
- Safe Rust paths: `slatec::blas::level3::strmm`

## Providers

- Canonical provider: `lin/strmm.f` (`relocated-subset`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/lin/strmm.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/lin/strmm.f) — `verified_cached`
...
- Exact Netlib source: [STRMM](https://www.netlib.org/slatec/lin/strmm.f)

### Arguments

| # | Argument | Direction | Role | Fortran type | Rust raw type | Shape | Contract |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `SIDE` | `input` | `scalar` | `CHARACTER` | `*mut core::ffi::c_char` | scalar | CHARACTER*1. On entry,  SIDE specifies whether  op( A ) multiplies B from the left or right as follows: = alpha*op( A )*B. = alpha*B*op( A ). Unchanged on exit. 'L' or 'l'  and is  n  when  SIDE = 'R' or 'r'. 'L' or 'l'  then 'R' or 'r' then LDA must be at least max( 1, n ). Unchanged on exit. |
...
| 8 | `A` | `input` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (LDA, *) | A   or   op( A ) = A'. Parameters ========== is an upper or lower triangular matrix as follows: is unit triangular as follows: REAL             array of DIMENSION ( LDA, k ), where k is m is not referenced. is not referenced. are not referenced either,  but are assumed to be  unity. Unchanged on exit. set of level 3 basic linear algebra subprograms. ACM TOMS, Vol. 16, No. 1, pp. 1-17, March 1990. |
...
| 10 | `B` | `input-output` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (LDB, *) | = alpha*op( A )*B,   or   B := alpha*B*op( A ), where  alpha  is a scalar,  B  is an m by n matrix,  A  is a unit, or non-unit,  upper or lower triangular matrix  and  op( A )  is one  of = alpha*op( A )*B. = alpha*B*op( A ). Unchanged on exit. must be at least zero. Unchanged on exit. must be at least zero. Unchanged on exit. REAL             array of DIMENSION ( LDB, n ). Before entry,  the leading  m by n part of the array  B must contain the matrix  B,  and  on exit  is overwritten  by the transformed matrix. |
...
Canonical Rust path: `slatec_sys::blas::level3::strmm`. Native symbol: `strmm_`. Declaration feature: `blas-level3`. Provider feature: `blas-level3`. ABI fingerprint: `unavailable`.
...
- Safe-wrapper status: `slatec::blas::level3::strmm`
