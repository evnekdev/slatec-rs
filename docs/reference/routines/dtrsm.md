...
| 10 | `B` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 2; dimensions (LDB, *) | are m by n matrices, A is a unit, or non-unit,  upper or lower triangular matrix  and  op( A )  is one  of must be at least zero. Unchanged on exit. must be at least zero. Unchanged on exit. DOUBLE PRECISION array of DIMENSION ( LDB, n ). Before entry,  the leading  m by n part of the array  B must contain  the  right-hand  side  matrix  B,  and  on exit  is overwritten by the solution matrix  X. |
