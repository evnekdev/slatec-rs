...
| 3 | `X` | `output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (N) | Double Precision X(N). Solution of (L*D*U)trans x = b. |
...
| 10 | `U` | `input` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | Double Precision U(NU). the unit upper triangular factor of the incomplete decomposition of some matrix stored in SLAP Column format. The diagonal of ones *IS* stored. This structure can be set up by the DSILUS routine. See the "Description", below for more details about the SLAP format. (NU is the number of non-zeros in the U array. |
