...
- Exact Netlib source: [DTPSV](https://www.netlib.org/slatec/lin/dtpsv.f)
...
| 6 | `X` | `input-output` | `array` | `DOUBLE PRECISION` | `*mut f64` | rank 1; dimensions (*) | DOUBLE PRECISION array of dimension at least ( 1 + ( n - 1 )*abs( INCX ) ). Before entry, the incremented array X must contain the n element right-hand side vector b. On exit, X is overwritten with the solution vector x. |
...
Canonical Rust path: `slatec_sys::blas::level2::dtpsv`. Native symbol: `dtpsv_`. Declaration feature: `blas-level2`. Provider feature: `blas-level2`. ABI fingerprint: `unavailable`.
