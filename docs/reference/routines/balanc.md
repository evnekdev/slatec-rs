...
| 3 | `A` | `input-output` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (NM, *) | contains the input matrix to be balanced. A is a two-dimensional REAL array, dimensioned A(NM,N). contains the balanced matrix. |
| 4 | `LOW` | `output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | two INTEGER variables such that A(I,J) is equal to zero if (1) I is greater than J and (2) J=1,. ,LOW-1 or I=IGH+1,. ,N. |
| 5 | `IGH` | `output` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | two INTEGER variables such that A(I,J) is equal to zero if (1) I is greater than J and (2) J=1,. ,LOW-1 or I=IGH+1,. ,N. |
