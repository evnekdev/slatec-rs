...
- Canonical provider: `main-src/src/sdcst.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/sdcst.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/sdcst.f) — `verified_cached`
...
| `EL` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (13, 12) | The array EL determines the basic method. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `TQ` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 2; dimensions (3, 12) | The array TQ is involved in adjusting the step size in relation to truncation error. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
