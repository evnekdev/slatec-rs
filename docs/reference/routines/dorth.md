...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `VNEW` | output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (*) | This routine orthogonalizes the vector VNEW against the previous KMP vectors in the V array. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `V` | output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 2; dimensions (N, *) | This routine orthogonalizes the vector VNEW against the previous KMP vectors in the V array. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `HES` | output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 2; dimensions (LDHES, *) | *Usage: INTEGER N, LL, LDHES, KMP DOUBLE PRECISION VNEW(N), V(N,LL), HES(LDHES,LL), SNORMW CALL DORTH(VNEW, V, HES, N, LL, LDHES, KMP, SNORMW) *Arguments: VNEW :INOUT Double Precision VNEW(N) On input, the vector of length N containing a scaled product of the Jacobian and the vector V(*,LL). | *Usage: INTEGER N, LL, LDHES, KMP DOUBLE PRECISION VNEW(N), V(N,LL), HES(LDHES,LL), SNORMW CALL DORTH(VNEW, V, HES, N, LL, LDHES, KMP, SNORMW) *Arguments: VNEW :INOUT Double Precision VNEW(N) On input, the vector of length N containing a scaled product of the Jacobian and the vector V(*,LL). Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
| `N` | output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | *Usage: INTEGER N, LL, LDHES, KMP DOUBLE PRECISION VNEW(N), V(N,LL), HES(LDHES,LL), SNORMW CALL DORTH(VNEW, V, HES, N, LL, LDHES, KMP, SNORMW) *Arguments: VNEW :INOUT Double Precision VNEW(N) On input, the vector of length N containing a scaled product of the Jacobian and the vector V(*,LL). | *Usage: INTEGER N, LL, LDHES, KMP DOUBLE PRECISION VNEW(N), V(N,LL), HES(LDHES,LL), SNORMW CALL DORTH(VNEW, V, HES, N, LL, LDHES, KMP, SNORMW) *Arguments: VNEW :INOUT Double Precision VNEW(N) On input, the vector of length N containing a scaled product of the Jacobian and the vector V(*,LL). Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `SNORMW` | output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | scalar | *Usage: INTEGER N, LL, LDHES, KMP DOUBLE PRECISION VNEW(N), V(N,LL), HES(LDHES,LL), SNORMW CALL DORTH(VNEW, V, HES, N, LL, LDHES, KMP, SNORMW) *Arguments: VNEW :INOUT Double Precision VNEW(N) On input, the vector of length N containing a scaled product of the Jacobian and the vector V(*,LL). | *Usage: INTEGER N, LL, LDHES, KMP DOUBLE PRECISION VNEW(N), V(N,LL), HES(LDHES,LL), SNORMW CALL DORTH(VNEW, V, HES, N, LL, LDHES, KMP, SNORMW) *Arguments: VNEW :INOUT Double Precision VNEW(N) On input, the vector of length N containing a scaled product of the Jacobian and the vector V(*,LL). Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
