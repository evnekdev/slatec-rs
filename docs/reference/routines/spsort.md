...
SPSORT returns the permutation vector IPERM generated by sorting the array X and, optionally, rearranges the values in X. X may be sorted in increasing or decreasing order. A slightly modified quicksort algorithm is used. IPERM is such that X(IPERM(I)) is the Ith value in the rearrangement of X. IPERM may be applied to another array by calling IPPERM, SPPERM, DPPERM or HPPERM. The main difference between SPSORT and its active sorting equivalent SSORT is that the data are referenced indirectly rather than directly. Therefore, SPSORT should require approximately twice as long to execute as SSORT. However, SPSORT is more general.
...
- Canonical provider: `main-src/src/spsort.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/spsort.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/spsort.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/spsort.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `X` | unavailable | `REAL` (`explicit`) | `*mut f32` | rank 1; dimensions (*) | SPSORT returns the permutation vector IPERM generated by sorting the array X and, optionally, rearranges the values in X. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `IPERM` | unavailable | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | rank 1; dimensions (*) | SPSORT returns the permutation vector IPERM generated by sorting the array X and, optionally, rearranges the values in X. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
