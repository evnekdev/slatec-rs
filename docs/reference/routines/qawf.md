...
| 5 | `EPSABS` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Real Absolute accuracy requested, EPSABS.GT.0. 6. ON RETURN or LIMLST.LT.1 or |
...
| 7 | `ABSERR` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | Real Estimate of the modulus of the absolute error, Which should equal or exceed ABS(I-RESULT) are set to zero. = 7 Bad integrand behaviour occurs within one or more of the cycles. Location and |
...
| 16 | `WORK` | `workspace` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | ARRAYS Real Vector of dimension at least On return, contain the integral contain the integral approximations over the cycles, approximations over the cycles, ..., WORK(LIMLST+LST) contain the error estimates over the cycles. further elements of WORK have no specific meaning for the user. |
