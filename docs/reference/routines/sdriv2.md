...
- Exact Netlib source: [SDRIV2](https://www.netlib.org/slatec/src/sdriv2.f)
...
| 2 | `T` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Mutable independent variable: initial point on the first call and returned solution/root point thereafter. |
| 3 | `Y` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Mutable length-at-least-`N` solution vector shared with the RHS and optional root callbacks. |
...
| 5 | `TOUT` | `input-output` | `scalar` | `REAL` | `*mut f32` | scalar | Input requested output point for the current continuation call. |
...
| 8 | `EPS` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Input/output relative accuracy request; the routine may raise a too-small value. |
| 9 | `EWT` | `input` | `scalar` | `REAL` | `*mut f32` | scalar | Input error-weight scale used to form `max(abs(Y(I)), EWT)` for the selected error control. |
...
| 11 | `WORK` | `workspace-output` | `workspace` | `REAL` | `*mut f32` | rank 1; dimensions (*) | Mutable persistent real workspace. Its minimum is `16*N + 2*NROOT + 250` for `MINT=1`, `N*N + 10*N + 2*NROOT + 250` for `2`, or `N*N + 17*N + 2*NROOT + 250` for `3`. |
...
Canonical Rust path: `slatec_sys::ode::sdriv2`. Native symbol: `sdriv2_`. Declaration feature: `ode-sdrive-expert`. Provider feature: `ode-sdrive-expert`. ABI fingerprint: `unavailable`.
...
- Safe-wrapper status: `slatec::ode::Driv2Session::<f32>::integrate_to_with_events`
