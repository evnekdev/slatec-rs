...
- Canonical provider: `main-src/src/dpchng.f` (`pinned-reproducible-subset`)
- Alternate providers:
  - `src/dpchng.f` (`live-main-source`)

## Official references

- [Netlib source](https://www.netlib.org/slatec/src/dpchng.f) — `verified_cached`
- [Netlib full source](https://www.netlib.org/cgi-bin/netlibfiles.pl?filename=/slatec/src/dpchng.f) — `verified_cached`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `II` | unavailable | `INTEGER` (`implicit_rule`) | `*mut crate::FortranInteger` | scalar | SUBROUTINE DPCHNG CHANGES ELEMENT II IN VECTOR +/- IRCX TO THE VALUE XVAL. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `SX` | workspace | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 1; dimensions (*) | SX(*),IX(*) THE WORK ARRAYS WHICH ARE USED TO STORE THE SPARSE MATRIX. | none stated in the separable source sentence Leading dimension: not established Workspace: SX(*),IX(*) THE WORK ARRAYS WHICH ARE USED TO STORE THE SPARSE MATRIX. | required; null is not permitted for an ordinary Fortran actual argument |
