...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `A` | input/output | `DOUBLE PRECISION` (`explicit`) | `*mut f64` | rank 2; dimensions (LDA, *) | DGEFA factors a double precision matrix by Gaussian elimination. | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
...
| `N` | input/output | `INTEGER` (`explicit`) | `*mut crate::FortranInteger` | scalar | (Time for DGECO) = (1 + 9/N)*(Time for DGEFA) . | none stated in the separable source sentence Leading dimension: not established Workspace: not established | required; null is not permitted for an ordinary Fortran actual argument |
