...
| 4 | `KODE` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | A parameter to indicate the scaling option 1 returns CY(L)=H(M,FNU+L-1,Z), L=1,. ,N =2 returns CY(L)=H(M,FNU+L-1,Z)*exp(-(3-2M)*Z*i),. |
| 5 | `M` | `input` | `scalar` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Superscript of Hankel function, M=1 or 2. |
...
| 9 | `NZ` | `status-output` | `status` | `INTEGER` | `*mut crate::FortranInteger` | scalar | Number of underflows set to zero NZ=0 Normal return NZ>0 CY(L)=0 for NZ values of L (if M=1 and Im(Z)>0 or if M=2 and Im(Z)<0, then CY(L)=0 for L=1,. ,NZ; in the com- plementary half planes, the underflows may not be in an uninterrupted sequence). |
