...
| 4 | `DET` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (2) | REAL(2) determinant of A or of TRANS(X)*X if requested. Otherwise not referenced. Determinant = DET(1) * 10. 0**DET(2) with 1. 0. LE. |
...
Canonical Rust path: `slatec_sys::linear_algebra::dense::spodi`. Native symbol: `spodi_`. Declaration feature: `linear-algebra`. Provider feature: `linear-algebra-real`. ABI fingerprint: `subroutine:void(mut_f32_ptr_rank2,mut_i32,mut_i32,mut_f32_ptr_rank1,mut_i32)`.
