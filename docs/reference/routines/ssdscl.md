...
| 7 | `X` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | X(N). Initial guess that will be later used in the iterative solution. of the scaled system. |
| 8 | `B` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | B(N). Right hand side vector. |
| 9 | `DINV` | `output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (N) | DINV(N). Upon return this array holds 1. /DIAG(A). This is an input if JOB = 0. |
