...
| 3 | `W` | `input-output` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | contains the diagonal elements of the real symmetric tridiagonal matrix. W is a one-dimensional REAL array, dimensioned W(N). contains the eigenvalues in ascending order. |
| 4 | `E` | `input` | `array` | `REAL` | `*mut f32` | rank 1; dimensions (*) | contains the subdiagonal elements of the matrix in its last N-1 positions. E(1) is arbitrary. E is a one-dimensional REAL array, dimensioned E(N). |
...
| 6 | `Z` | `output` | `array` | `REAL` | `*mut f32` | rank 2; dimensions (NM, *) | contains the eigenvectors if MATZ is not zero. The eigen- vectors are orthonormal. Z is a two-dimensional REAL array, dimensioned Z(NM,N). |
