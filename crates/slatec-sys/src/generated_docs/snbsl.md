...
This canonical unsafe binding exposes original SLATEC routine `SNBSL`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SNBSL](https://www.netlib.org/slatec/src/snbsl.f).

# Arguments

## 1. `ABE`

input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (LDA, *). REAL(LDA, NC) the output from SNBCO or SNBFA. NC must be .GE. 2*ML+MU+1 . REAL(LDA, NC) the output from SNBCO or SNBFA. NC must be .GE. 2*ML+MU+1 . not applicable or not stated by selected source not a workspace argument
...
input `array` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (*). INTEGER(N) the pivot vector from SNBCO or SNBFA. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- Original SLATEC routine: `SNBSL`
- Native symbol: `snbsl_`
...
- Exact Netlib source file: [SNBSL](https://www.netlib.org/slatec/src/snbsl.f)
