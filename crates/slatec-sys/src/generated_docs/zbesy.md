...
input `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). DOUBLE PRECISION work vector of dimension N DOUBLE PRECISION work vector of dimension N not applicable or not stated by selected source not a workspace argument
