...
This canonical unsafe binding exposes original SLATEC routine `SSYR`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSYR](https://www.netlib.org/slatec/lin/ssyr.f).
...
input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (LDA, *). = alpha*x*x' + A, where alpha is a real scalar, x is an n element vector and A is an REAL             array of DIMENSION ( LDA, n ). = alpha*x*x' + A, where alpha is a real scalar, x is an n element vector and A is an REAL             array of DIMENSION ( LDA, n ). not applicable or not stated by selected source not a workspace argument
...
- Exact Netlib source file: [SSYR](https://www.netlib.org/slatec/lin/ssyr.f)
