...
This canonical unsafe binding exposes original SLATEC routine `CSYRK`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [CSYRK](https://www.netlib.org/slatec/lin/csyrk.f).
...
input `scalar` argument; Fortran declaration `CHARACTER`, Rust ABI type `*mut core::ffi::c_char`, and scalar. CHARACTER*1. On entry,  TRANS  specifies the operation to be performed as follows: = alpha*A*A' + beta*C. = alpha*A'*A + beta*C. Unchanged on exit. 'N' or 'n',  K  specifies  the number of  columns   of  the   matrix   A,   and  on   entry   with 'T' or 't',  K  specifies  the number of rows of the 'N' or 'n',  and is  n  otherwise. 'N' or 'n',  the  leading  n by k part of the array  A  must contain the matrix  A,  otherwise 'N' or 'n' then  LDA must be at least  max( 1, n ), otherwise  LDA must be at least  max( 1, k ). Unchanged on exit. CHARACTER*1. On entry,  TRANS  specifies the operation to be performed as follows: = alpha*A*A' + beta*C. = alpha*A'*A + beta*C. Unchanged on exit. 'N' or 'n',  K  specifies  the number of  columns   of  the   matrix   A,   and  on   entry   with 'T' or 't',  K  specifies  the number of rows of the 'N' or 'n',  and is  n  otherwise. 'N' or 'n',  the  leading  n by k part of the array  A  must contain the matrix  A,  otherwise 'N' or 'n' then  LDA must be at least  max( 1, n ), otherwise  LDA must be at least  max( 1, k ). Unchanged on exit. not applicable or not stated by selected source not a workspace argument
...
input `scalar` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and scalar. is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== COMPLEX         . On entry, ALPHA specifies the scalar alpha. Unchanged on exit. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `scalar` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and scalar. is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== COMPLEX         . On entry, BETA specifies the scalar beta. Unchanged on exit. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 9. `C`

input-output `array` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and rank 2; dimensions (LDC, *). = alpha*A*A' + beta*C, or = alpha*A'*A + beta*C, is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== = alpha*A*A' + beta*C. = alpha*A'*A + beta*C. Unchanged on exit. must be at least zero. Unchanged on exit. COMPLEX          array of DIMENSION ( LDC, n ). = alpha*A*A' + beta*C, or = alpha*A'*A + beta*C, is an  n by n symmetric matrix and  A  is an  n by k  matrix in the first case and a  k by n  matrix in the second case. Parameters ========== = alpha*A*A' + beta*C. = alpha*A'*A + beta*C. Unchanged on exit. must be at least zero. Unchanged on exit. COMPLEX          array of DIMENSION ( LDC, n ). not applicable or not stated by selected source not a workspace argument
...
- Native symbol: `csyrk_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [CSYRK](https://www.netlib.org/slatec/lin/csyrk.f)
