...
This canonical unsafe binding exposes original SLATEC routine `SSILUR`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSILUR](https://www.netlib.org/slatec/lin/ssilur.f).
...
Integer. Length of the real workspace, RWORK. LENW >= NL+NU+4*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). NU is the number of non-zeros in the upper triangle of the.
...
Integer. Length of integer workspace, IWORK. LENIW >= NL+NU+4*N+10. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). NU is the number of non-zeros in the upper triangle of the.
...
- Original SLATEC routine: `SSILUR`
- Native symbol: `ssilur_`
...
- Exact Netlib source file: [SSILUR](https://www.netlib.org/slatec/lin/ssilur.f)
