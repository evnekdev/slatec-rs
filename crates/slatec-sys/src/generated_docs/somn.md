...
**Direction:** `workspace-output`. **Fortran type:** `REAL`. **Rust ABI type:** `*mut f32`. **Shape:** rank 2; dimensions (N, 0:NSAVE).
...
**Direction:** `workspace-output`. **Fortran type:** `REAL`. **Rust ABI type:** `*mut f32`. **Shape:** rank 1; dimensions (NSAVE).

CSAV(NSAVE) Real arrays used for workspace.
...
- `CSAV`: CSAV(NSAVE) Real arrays used for workspace.
