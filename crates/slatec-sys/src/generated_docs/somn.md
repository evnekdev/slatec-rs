...
This canonical unsafe binding exposes original SLATEC routine `SOMN`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SOMN](https://www.netlib.org/slatec/lin/somn.f).

# Arguments

## 1. `N`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. NSAVE), AP(N,0:NSAVE), EMAP(N,0:NSAVE) REAL     DZ(N), CSAV(NSAVE), RWORK(USER DEFINED) EXTERNAL MATVEC, MSOLVE CALL SOMN(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, $     NSAVE, ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, R, $     Z, P, AP, EMAP, DZ, CSAV, RWORK, IWORK) IN       Integer. Order of the Matrix. hand side vector and Z is the solution upon return.  NELT, IA, JA, A and is the number of columns in the matrix and not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (N, 0:NSAVE). NSAVE), AP(N,0:NSAVE), EMAP(N,0:NSAVE) REAL     DZ(N), CSAV(NSAVE), RWORK(USER DEFINED) EXTERNAL MATVEC, MSOLVE CALL SOMN(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, $     NSAVE, ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, R, $     Z, P, AP, EMAP, DZ, CSAV, RWORK, IWORK) < epsilon**2. WORK     Real P(N,0:NSAVE). not stated by selected source not applicable or not stated by selected source not a workspace argument

## 22. `AP`

input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (N, 0:NSAVE). < epsilon**2. WORK     Real AP(N,0:NSAVE). not stated by selected source not applicable or not stated by selected source not a workspace argument

## 23. `EMAP`

input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (N, 0:NSAVE). WORK     Real EMAP(N,0:NSAVE). not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (NSAVE). WORK     Real CSAV(NSAVE) Real arrays used for workspace. not stated by selected source not applicable or not stated by selected source
...
- `CSAV`: WORK     Real CSAV(NSAVE) Real arrays used for workspace.
...
- Exact Netlib source file: [SOMN](https://www.netlib.org/slatec/lin/somn.f)
