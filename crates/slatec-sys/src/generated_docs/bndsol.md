...
**Direction:** `input-output`. **Fortran type:** `REAL`. **Rust ABI type:** `*mut f32`. **Shape:** rank 2; dimensions (MDG, *).

Description of calling sequence for BNDACC. The entire set of parameters for BNDACC are The working array into which the user will place the MT by NB+1 block (C F) in rows IR through IR+MT-1, columns 1 through NB+1. See descriptions of IR and MT below. The value of MDG should be. GE. MU.
...
The bandwidth of the data matrix A. contents as following the last call to BNDACC.
...
The number of variables in the solution vector. If any of the N diagonal terms are zero the subroutine BNDSOL prints an appropriate message. This condition is considered an error.
