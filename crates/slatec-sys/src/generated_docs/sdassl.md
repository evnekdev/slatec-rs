...
Solves a differential/algebraic system `G(T, Y, YPRIME) = 0` with backward-differentiation formulas of orders one through five. This is the original single precision SLATEC DASSL driver `SDASSL`.

# Description

The first call starts a new problem. It requires consistent initial `T`, `Y`, and `YPRIME` unless `INFO(11)=1` asks DASSL to compute a consistent derivative. Subsequent calls continue the same problem, retain state in `RWORK` and `IWORK`, and advance in the original direction of `TOUT`. The verified source prologue is [SDASSL](https://www.netlib.org/slatec/src/sdassl.f).

# Arguments

- `RES`: required `DasslResidualF32` callback. It implements `RES(T, Y, YPRIME, DELTA, IRES, RPAR, IPAR)`, writes `DELTA[0..NEQ] = G(T, Y, YPRIME)`, and must not alter `T`, `Y`, or `YPRIME`. It receives `IRES=0`; it may set `IRES=-1` for an illegal input (DASSL retries) or `IRES=-2` to return with `IDID=-11`.
...
- `RWORK`: mutable f32 workspace of length `LRW`; it contains persistent continuation state.
...
- Original SLATEC routine: `SDASSL`
- Native symbol: `sdassl_`
- Precision: single precision
- Exact Netlib source file: [SDASSL](https://www.netlib.org/slatec/src/sdassl.f)
