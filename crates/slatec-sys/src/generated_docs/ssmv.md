...
This canonical unsafe binding exposes original SLATEC routine `SSMV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSMV](https://www.netlib.org/slatec/lin/ssmv.f).
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (N). IN       Real X(N). The vector that should be multiplied by the matrix. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 3. `Y`

input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (N). A*X. OUT      Real Y(N). The product of the matrix and the vector. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- Exact Netlib source file: [SSMV](https://www.netlib.org/slatec/lin/ssmv.f)
