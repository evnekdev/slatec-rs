...
Subroutine CGEIR solves a general NxN system of complex linear equations using LINPACK subroutines CGEFA and CGESL. One pass of iterative refinement is used only to obtain an estimate of the accuracy. That is, if A is an NxN complex
...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. vectors, then CGEIR solves the equation must not have been altered by the 1) INTEGER the order of the matrix A.  The first N elements of the array A are the elements of the first column of must be greater than or equal to 1. vectors, then CGEIR solves the equation must not have been altered by the 1) INTEGER the order of the matrix A.  The first N elements of the array A are the elements of the first column of must be greater than or equal to 1. not applicable or not stated by selected source not a workspace argument
...
workspace `workspace` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and rank 2; dimensions (N, *). must not have been altered by the COMPLEX(N*(N+1)) must not have been altered by the COMPLEX(N*(N+1)) not applicable or not stated by selected source
...
- `WORK`: must not have been altered by the COMPLEX(N*(N+1))
