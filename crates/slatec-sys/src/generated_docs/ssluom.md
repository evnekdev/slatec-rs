...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Length of the real workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1) IN       Integer. Length of the real workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1) not applicable or not stated by selected source
...
- `RWORK`: WORK     Real RWORK(LENW). Real array used for workspace.  NL is the number of non- zeros in the lower triangle of the matrix (including the diagonal).  NU is the number of non-zeros in the upper triangle of the matrix (including the diagonal).
- `LENW`: IN       Integer. Length of the real workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1)
...
- Canonical Rust path: `slatec_sys::linear_algebra::sparse::ssluom`
- Original SLATEC routine: `SSLUOM`
- Native symbol: `ssluom_`
...
- Exact Netlib source file: [SSLUOM](https://www.netlib.org/slatec/lin/ssluom.f)
