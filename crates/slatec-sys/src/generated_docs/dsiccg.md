...
This canonical unsafe binding exposes original SLATEC routine `DSICCG`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSICCG](https://www.netlib.org/slatec/lin/dsiccg.f).
...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Length of the double precision workspace, RWORK. NL+5*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). IN       Integer. Length of the double precision workspace, RWORK. NL+5*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). not applicable or not stated by selected source
...
- `LENW`: IN       Integer. Length of the double precision workspace, RWORK. NL+5*N. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal).
...
- Original SLATEC routine: `DSICCG`
- Native symbol: `dsiccg_`
...
- Exact Netlib source file: [DSICCG](https://www.netlib.org/slatec/lin/dsiccg.f)
