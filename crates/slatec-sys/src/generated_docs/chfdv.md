...
CHFDV: Cubic Hermite Function and Derivative Evaluator Evaluates the cubic polynomial determined by function values
...
input `scalar` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and scalar. and derivatives D1,D2 on interval (X1,X2), together with (input) values of function at X1 and X2, respectively. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (*). 1(1)NE. If only function values are required, use CHFEV, instead. (input) real array of points at which the functions are to be evaluated.  If any of the XE are outside the interval not stated by selected source not applicable or not stated by selected source not a workspace argument

## 9. `FE`

output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (*). (output) real array of values of the cubic function defined by  X1,X2, F1,F2, D1,D2  at the points  XE. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 10. `DE`

output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (*). (output) real array of values of the first derivative of the same function at the points  XE. not stated by selected source not applicable or not stated by selected source not a workspace argument
