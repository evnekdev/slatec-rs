...
A(NELT). These arrays contain the matrix data structure for A. It could take any form. See "Description", below, for more details. a flag which, if non-zero, denotes that A is symmetric and only the lower or upper triangle is stored.
...
ATDZ(N). arrays used for workspace.
...
- `ATDZ`: ATDZ(N). arrays used for workspace.
