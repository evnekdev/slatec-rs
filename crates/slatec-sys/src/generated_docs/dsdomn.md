...
This canonical unsafe binding exposes original SLATEC routine `DSDOMN`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSDOMN](https://www.netlib.org/slatec/lin/dsdomn.f).

# Arguments

## 1. `N`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Order of the Matrix. NELT+1, where N is the number of columns not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Length of the double precision workspace, RWORK. 7*N+NSAVE*(3*N+1). IN       Integer. Length of the double precision workspace, RWORK. 7*N+NSAVE*(3*N+1). not applicable or not stated by selected source
...
- `LENW`: IN       Integer. Length of the double precision workspace, RWORK. 7*N+NSAVE*(3*N+1).
...
- Original SLATEC routine: `DSDOMN`
- Native symbol: `dsdomn_`
...
- Exact Netlib source file: [DSDOMN](https://www.netlib.org/slatec/lin/dsdomn.f)
