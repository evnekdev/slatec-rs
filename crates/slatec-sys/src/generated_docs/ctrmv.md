...
This canonical unsafe binding exposes original SLATEC routine `CTRMV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [CTRMV](https://www.netlib.org/slatec/lin/ctrmv.f).
...
input `array` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and rank 2; dimensions (LDA, *). unit, upper or lower triangular matrix. Parameters ========== is unit triangular as follows: COMPLEX          array of DIMENSION ( LDA, n ). is not referenced. is not referenced. are not referenced either, but are assumed to be unity. Unchanged on exit. unit, upper or lower triangular matrix. Parameters ========== is unit triangular as follows: COMPLEX          array of DIMENSION ( LDA, n ). is not referenced. is not referenced. are not referenced either, but are assumed to be unity. Unchanged on exit. not applicable or not stated by selected source not a workspace argument
...
- Native symbol: `ctrmv_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [CTRMV](https://www.netlib.org/slatec/lin/ctrmv.f)
