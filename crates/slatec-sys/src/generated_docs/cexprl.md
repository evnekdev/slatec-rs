...
input `scalar` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and scalar. 1)/Z .  For small ABS(Z), we use the Taylor series.  We could instead use the expression 1)/Z = (X*EXPREL(X) * (1 - 2*SIN(Y/2)**2) - 2*SIN(Y/2)**2 + I*SIN(Y)*(1+X*EXPREL(X))) / Z not stated by selected source not applicable or not stated by selected source not a workspace argument
