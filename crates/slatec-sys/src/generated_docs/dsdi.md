...
input `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). IN       Double Precision B(N). Vector to multiply the diagonal by. by.  This array must be set by the user or by a call to the SLAP routine DSDS or DSD2S.  The length of RWORK must be >= IWORK(4)+N. conditioner setup routines DSDS or DSD2S. IN       Double Precision B(N). Vector to multiply the diagonal by. by.  This array must be set by the user or by a call to the SLAP routine DSDS or DSD2S.  The length of RWORK must be >= IWORK(4)+N. conditioner setup routines DSDS or DSD2S. not applicable or not stated by selected source not a workspace argument

## 3. `X`

output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). DIAG*B, where DIAG is a diagonal matrix. OUT      Double Precision X(N). Result of DIAG*B. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (NELT). DUMMY    Double Precision A(NELT). not stated by selected source not applicable or not stated by selected source not a workspace argument
...
workspace `workspace` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (*). IN       Double Precision RWORK(USER DEFINED). Work array holding the diagonal of some matrix to scale not stated by selected source not applicable or not stated by selected source
