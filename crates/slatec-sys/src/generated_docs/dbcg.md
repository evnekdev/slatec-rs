...
This canonical unsafe binding exposes original SLATEC routine `DBCG`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DBCG](https://www.netlib.org/slatec/lin/dbcg.f).
...
callback `callback` argument; Fortran declaration `INTEGER`, Rust ABI type `reviewed unsafe extern callback function pointer`, and scalar. EXT      External. CALL MSOLVE(N, R, Z, NELT, IA, JA, A, ISYM, RWORK, IWORK) is an integer work array for the same purpose as RWORK. and MTSOLV. The callback must remain valid for the complete native call, satisfy the exact reviewed ABI, and must not unwind into Fortran. not stated by selected source not applicable or not stated by selected source
...
input-output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). hand side vector, and Z is the solution upon return.  NELT,  IA, JA, A WORK     Double Precision R(N). not stated by selected source not applicable or not stated by selected source not a workspace argument

## 21. `Z`

input `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). WORK     Double Precision Z(N). not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input-output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). is supplied via RWORK and IWORK arrays).  The name of the MTSOLV routine must be declared external in the calling program.  The call- hand side vector, and ZZ is the solution upon return.  NELT, IA, JA, A WORK     Double Precision RR(N). is supplied via RWORK and IWORK arrays).  The name of the MTSOLV routine must be declared external in the calling program.  The call- hand side vector, and ZZ is the solution upon return.  NELT, IA, JA, A WORK     Double Precision RR(N). not applicable or not stated by selected source

## 24. `ZZ`

input `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). RR for is supplied via RWORK and IWORK arrays).  The name of the MTSOLV routine must be declared external in the calling program.  The call- WORK     Double Precision ZZ(N). RR for is supplied via RWORK and IWORK arrays).  The name of the MTSOLV routine must be declared external in the calling program.  The call- WORK     Double Precision ZZ(N). not applicable or not stated by selected source

## 25. `PP`

input-output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (N). WORK     Double Precision PP(N). 155. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- `MSOLVE`: EXT      External. CALL MSOLVE(N, R, Z, NELT, IA, JA, A, ISYM, RWORK, IWORK) is an integer work array for the same purpose as RWORK. and MTSOLV.
...
- `RR`: is supplied via RWORK and IWORK arrays).  The name of the MTSOLV routine must be declared external in the calling program.  The call- hand side vector, and ZZ is the solution upon return.  NELT, IA, JA, A WORK     Double Precision RR(N).
- `ZZ`: RR for is supplied via RWORK and IWORK arrays).  The name of the MTSOLV routine must be declared external in the calling program.  The call- WORK     Double Precision ZZ(N).
...
- Exact Netlib source file: [DBCG](https://www.netlib.org/slatec/lin/dbcg.f)
