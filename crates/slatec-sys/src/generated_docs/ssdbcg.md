...
Usage: INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX INTEGER ITER, IERR, IUNIT, LENW, IWORK(10), LENIW REAL B(N), X(N), A(NELT), TOL, ERR, RWORK(8*N) CALL SSDBCG(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL, $ ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW ) This routine performs preconditioned BiConjugate gradient method on the Non-Symmetric positive definite linear system

# Description

This canonical unsafe binding exposes original SLATEC routine `SSDBCG`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSDBCG](https://www.netlib.org/slatec/lin/ssdbcg.f).
...
- `IWORK`: WORK     Integer IWORK(LENIW). Used to hold pointers into the RWORK array. 10. Upon return the following locations of IWORK hold information which may be of use to the user: Amount of Integer workspace actually used. Amount of Real workspace actually used.
- `LENIW`: not a workspace argument

# ABI notes

- Canonical Rust path: `slatec_sys::linear_algebra::sparse::ssdbcg`
- Original SLATEC routine: `SSDBCG`
- Native symbol: `ssdbcg_`
...
- Exact Netlib source file: [SSDBCG](https://www.netlib.org/slatec/lin/ssdbcg.f)
