...
IWORK(LENIW). Used to hold pointers into the RWORK array. Amount of Integer workspace actually used. IWORK(10) Amount of Double Precision workspace actually used.

## `LENIW`

**Direction:** `output`. **Fortran type:** `INTEGER`. **Rust ABI type:** `*mut crate::FortranInteger`. **Shape:** scalar.

Integer. Length of the integer workspace, IWORK. LENIW >= 10. Upon return the following locations of IWORK hold information which may be of use to the user:.
