...
This canonical unsafe binding exposes original SLATEC routine `DSDBCG`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSDBCG](https://www.netlib.org/slatec/lin/dsdbcg.f).
...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Length of the double precision workspace, RWORK. 8*N. IN       Integer. Length of the double precision workspace, RWORK. 8*N. not applicable or not stated by selected source
...
- `LENW`: IN       Integer. Length of the double precision workspace, RWORK. 8*N.
...
- Original SLATEC routine: `DSDBCG`
- Native symbol: `dsdbcg_`
...
- Exact Netlib source file: [DSDBCG](https://www.netlib.org/slatec/lin/dsdbcg.f)
