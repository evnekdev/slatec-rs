...
This canonical unsafe binding exposes original SLATEC routine `CTPMV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [CTPMV](https://www.netlib.org/slatec/lin/ctpmv.f).
...
= A*x, or x := A'*x, or x := conjg( A')*x, where x is an n element vector and A is an n by n unit, or non-unit, upper or lower triangular matrix, supplied in packed form. = A*x. COMPLEX array of dimension at least ( 1 + ( n - 1 )*abs( INCX ) ). Before entry, the incremented array X must contain the n element vector x. On exit, X is overwritten with the transformed vector x.
...
- Native symbol: `ctpmv_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [CTPMV](https://www.netlib.org/slatec/lin/ctpmv.f)
