...
This canonical unsafe binding exposes original SLATEC routine `DSLUOM`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSLUOM](https://www.netlib.org/slatec/lin/dsluom.f).
...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Length of the double precision workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1) IN       Integer. Length of the double precision workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1) not applicable or not stated by selected source
...
- `LENW`: IN       Integer. Length of the double precision workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1)
...
- Original SLATEC routine: `DSLUOM`
- Native symbol: `dsluom_`
...
- Exact Netlib source file: [DSLUOM](https://www.netlib.org/slatec/lin/dsluom.f)
