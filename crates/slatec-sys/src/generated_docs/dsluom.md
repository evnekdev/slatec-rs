...
Integer. Number of direction vectors to save and orthogonalize against.
...
Double Precision RWORK(LENW). Double Precision array used for workspace. NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal). NU is the number of non-zeros in the upper.

## `LENW`

**Direction:** `input`. **Fortran type:** `INTEGER`. **Rust ABI type:** `*mut crate::FortranInteger`. **Shape:** scalar.

Integer. Length of the double precision workspace, RWORK. NL+NU+4*N+NSAVE*(3*N+1).
...
Integer. Length of the integer workspace, IWORK. NL+NU+4*N+12.
