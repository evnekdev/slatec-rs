...
This canonical unsafe binding exposes original SLATEC routine `CHPDI`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [CHPDI](https://www.netlib.org/slatec/lin/chpdi.f).

# Arguments

## 1. `AP`

input-output `array` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and rank 1; dimensions (*). COMPLEX (N*(N+1)/2) the output from CHPFA. contains the upper triangle of the inverse of the original matrix, stored in packed form. The columns of the upper triangle are stored sequentially in a one-dimensional array. COMPLEX (N*(N+1)/2) the output from CHPFA. contains the upper triangle of the inverse of the original matrix, stored in packed form. The columns of the upper triangle are stored sequentially in a one-dimensional array. not applicable or not stated by selected source not a workspace argument
...
A division by zero will occur if the inverse is requested and  CHPCO  has set RCOND .EQ. 0.0 or  CHPFA  has set  INFO .NE. 0 .
...
- Original SLATEC routine: `CHPDI`
- Native symbol: `chpdi_`
- ABI fingerprint: `subroutine:void(mut_complex32_array_rank1,mut_i32,mut_i32_array_rank1,mut_f32_array_rank1,mut_i32_array_rank1,mut_complex32_array_rank1,mut_i32)`
- Exact Netlib source file: [CHPDI](https://www.netlib.org/slatec/lin/chpdi.f)
