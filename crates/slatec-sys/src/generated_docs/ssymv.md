...
This canonical unsafe binding exposes original SLATEC routine `SSYMV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSYMV](https://www.netlib.org/slatec/lin/ssymv.f).
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (LDA, *). is an n by n symmetric matrix. Parameters ========== REAL             array of DIMENSION ( LDA, n ). is an n by n symmetric matrix. Parameters ========== REAL             array of DIMENSION ( LDA, n ). not applicable or not stated by selected source not a workspace argument
...
input `scalar` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and scalar. are scalars, x and y are n element vectors and REAL            . On entry, BETA specifies the scalar beta. When BETA is supplied as zero then Y need not be set on input. Unchanged on exit. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 9. `Y`

input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (*). = alpha*A*x + beta*y, REAL             array of dimension at least is overwritten by the updated is overwritten by the updated vector y. vector y. must not be zero. Unchanged on exit. = alpha*A*x + beta*y, REAL             array of dimension at least is overwritten by the updated is overwritten by the updated vector y. vector y. must not be zero. Unchanged on exit. not applicable or not stated by selected source not a workspace argument
...
- Native symbol: `ssymv_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [SSYMV](https://www.netlib.org/slatec/lin/ssymv.f)
