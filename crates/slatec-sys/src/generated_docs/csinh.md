...
input `scalar` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and scalar. calculates the complex hyperbolic sine of complex is in units of radians. is in units of radians. not stated by selected source not applicable or not stated by selected source not a workspace argument
