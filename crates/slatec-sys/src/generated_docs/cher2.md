...
This canonical unsafe binding exposes original SLATEC routine `CHER2`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [CHER2](https://www.netlib.org/slatec/lin/cher2.f).
...
input `scalar` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and scalar. are n element vectors and A is an n by n hermitian matrix. Parameters ========== COMPLEX         . On entry, ALPHA specifies the scalar alpha. Unchanged on exit. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 4. `X`

input `array` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and rank 1; dimensions (*). are n element vectors and A is an n by n hermitian matrix. Parameters ========== COMPLEX          array of dimension at least must not be zero. Unchanged on exit. are n element vectors and A is an n by n hermitian matrix. Parameters ========== COMPLEX          array of dimension at least must not be zero. Unchanged on exit. not applicable or not stated by selected source not a workspace argument
...
input-output `array` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and rank 2; dimensions (LDA, *). = alpha*x*conjg( y') + conjg( alpha)*y*conjg( x') + A, are n element vectors and A is an n by n hermitian matrix. Parameters ========== COMPLEX          array of DIMENSION ( LDA, n ). = alpha*x*conjg( y') + conjg( alpha)*y*conjg( x') + A, are n element vectors and A is an n by n hermitian matrix. Parameters ========== COMPLEX          array of DIMENSION ( LDA, n ). not applicable or not stated by selected source not a workspace argument
...
- Native symbol: `cher2_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [CHER2](https://www.netlib.org/slatec/lin/cher2.f)
