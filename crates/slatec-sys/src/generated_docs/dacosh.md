...
input `scalar` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and scalar. calculates the double precision arc hyperbolic cosine for double precision argument X.  The result is returned on the positive branch. not stated by selected source not applicable or not stated by selected source not a workspace argument
