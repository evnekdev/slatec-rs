...
This canonical unsafe binding exposes original SLATEC routine `DQAGIE`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DQAGIE](https://www.netlib.org/slatec/src/dqagie.f).
...
input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. Double precision Indicating the kind of integration range involved 1 corresponds to  (BOUND,+INFINITY), 1            to  (-INFINITY,BOUND), INFINITY,+INFINITY). not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input-output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (*). are set to 0 and 1 respectively. Double precision Vector of dimension at least LIMIT, the first are set to 0 and 1 respectively. Double precision Vector of dimension at least LIMIT, the first not applicable or not stated by selected source not a workspace argument
...
input-output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (*). are set to zero. Double precision Vector of dimension at least LIMIT,  the first are set to zero. Double precision Vector of dimension at least LIMIT,  the first not applicable or not stated by selected source not a workspace argument
...
- Original SLATEC routine: `DQAGIE`
- Native symbol: `dqagie_`
- ABI fingerprint: `subroutine:void(fn:f64(ref_f64),mut_f64,mut_i32,mut_f64,mut_f64,mut_i32,mut_f64,mut_f64,mut_i32,mut_i32,mut_f64_ptr_rank1,mut_f64_ptr_rank1,mut_f64_ptr_rank1,mut_f64_ptr_rank1,mut_i32_ptr_rank1,mut_i32)`
- Exact Netlib source file: [DQAGIE](https://www.netlib.org/slatec/src/dqagie.f)
