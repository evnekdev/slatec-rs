...
This canonical unsafe binding exposes original SLATEC routine `SGEMV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SGEMV](https://www.netlib.org/slatec/lin/sgemv.f).
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (LDA, *). is an REAL             array of DIMENSION ( LDA, n ). Before entry, the leading m by n part of the array A must contain the matrix of coefficients. Unchanged on exit. is an REAL             array of DIMENSION ( LDA, n ). Before entry, the leading m by n part of the array A must contain the matrix of coefficients. Unchanged on exit. not applicable or not stated by selected source not a workspace argument
...
- Native symbol: `sgemv_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [SGEMV](https://www.netlib.org/slatec/lin/sgemv.f)
