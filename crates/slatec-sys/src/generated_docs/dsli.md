...
This canonical unsafe binding exposes original SLATEC routine `DSLI`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSLI](https://www.netlib.org/slatec/lin/dsli.f).
...
workspace `workspace` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (10). NEL Starting location of IEL in IWORK. Starting location of JEL in IWORK. Starting location of EL in RWORK. See the DESCRIPTION of DSLI2 for details. not stated by selected source not applicable or not stated by selected source
...
- Exact Netlib source file: [DSLI](https://www.netlib.org/slatec/lin/dsli.f)
