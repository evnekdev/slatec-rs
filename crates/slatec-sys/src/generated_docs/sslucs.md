...
workspace `workspace` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (LENW). WORK     Real RWORK(LENW). Real array used for workspace.  NL is the number of non- zeros in the lower triangle of the matrix (including the diagonal).  NU is the number of non-zeros in the upper triangle of the matrix (including the diagonal). NL+NU+8*N. not stated by selected source not applicable or not stated by selected source

## 17. `LENW`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. NL+NU+8*N. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- `RWORK`: WORK     Real RWORK(LENW). Real array used for workspace.  NL is the number of non- zeros in the lower triangle of the matrix (including the diagonal).  NU is the number of non-zeros in the upper triangle of the matrix (including the diagonal). NL+NU+8*N.
...
- Original SLATEC routine: `SSLUCS`
- Native symbol: `sslucs_`
...
- Exact Netlib source file: [SSLUCS](https://www.netlib.org/slatec/lin/sslucs.f)
