...
This canonical unsafe binding exposes original SLATEC routine `DGBSL`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DGBSL](https://www.netlib.org/slatec/lin/dgbsl.f).

# Arguments

## 1. `ABD`

input `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 2; dimensions (LDA, *). DOUBLE PRECISION(LDA, N) the output from DGBCO or DGBFA. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `array` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (*). INTEGER(N) the pivot vector from DGBCO or DGBFA. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- Original SLATEC routine: `DGBSL`
- Native symbol: `dgbsl_`
...
- Exact Netlib source file: [DGBSL](https://www.netlib.org/slatec/lin/dgbsl.f)
