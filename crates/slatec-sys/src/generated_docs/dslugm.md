...
Integer. Maximum number of iterations. This routine uses the default of NRMAX = ITMAX/NSAVE to determine the when each restart should occur. See the description of NRMAX and MAXL in DGMRES for a full and frightfully interesting discussion of this topic.
...
Integer. Length of the double precision workspace, RWORK. 1 + N*(NSAVE+7) + NSAVE*(NSAVE+3)+NL+NU. Here NL is the number of non-zeros in the lower triangle of the matrix (including the diagonal) and NU is the number of non-zeros in the upper triangle of the matrix (including the diagonal). For the recommended values, RWORK has size at least 131 + 17*N + NL + NU.

## `IWORK`

**Direction:** `workspace-output`. **Fortran type:** `INTEGER`. **Rust ABI type:** `*mut crate::FortranInteger`. **Shape:** rank 1; dimensions (LENIW).

IWORK(LENIW). Used to hold pointers into the RWORK array. Upon return the following locations of IWORK hold information which may be of use to the user: Amount of Integer workspace actually used. IWORK(10) Amount of Double Precision workspace actually used.
...
- Original SLATEC routine: `DSLUGM`
- Native symbol: `dslugm_`
...
- Exact Netlib source file: [DSLUGM](https://www.netlib.org/slatec/lin/dslugm.f)
