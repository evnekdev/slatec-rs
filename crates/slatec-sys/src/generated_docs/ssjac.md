...
Usage: INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX INTEGER ITER, IERR, IUNIT, LENW, IWORK(LENIW), LENIW REAL B(N), X(N), A(NELT), TOL, ERR, RWORK(LENW) CALL SSJAC(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL, $ ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW ) Jacobi's method solves the linear system Ax=b with the

# Description

This canonical unsafe binding exposes original SLATEC routine `SSJAC`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSJAC](https://www.netlib.org/slatec/lin/ssjac.f).

# Arguments

## 1. `N`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. Order of the Matrix. 1       n    n 1       n NELT+1, where  N  is the number of columns in  the not stated by selected source not applicable or not stated by selected source not a workspace argument

## 2. `B`

input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (N). IN       Real B(N). Right-hand side vector. LX - UX ) AX ) The Sparse Linear Algebra Package (SLAP) utilizes two matrix data structures: 1) the  SLAP Triad  format or  2)  the SLAP Column format.  The user can hand this routine either of the of these data structures and SLAP  will figure out which one is being used and act accordingly. =================== S L A P Triad format =================== This routine requires that the  matrix A be   stored in  the SLAP  Triad format.  In  this format only the non-zeros  are stored.  They may appear in  *ANY* order.  The user supplies not stated by selected source not applicable or not stated by selected source not a workspace argument
...
workspace `workspace` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (LENW). WORK     Real RWORK(LENW). Real array used for workspace. 4*N. not stated by selected source not applicable or not stated by selected source
...
- `RWORK`: WORK     Real RWORK(LENW). Real array used for workspace. 4*N.
- `LENW`: not a workspace argument
- `IWORK`: WORK     Integer IWORK(LENIW). Used to hold pointers into the real workspace, RWORK. Upon return the following locations of IWORK hold information which may be of use to the user: Amount of Integer workspace actually used. Amount of Real workspace actually used. 10.
- `LENIW`: not a workspace argument

# ABI notes

- Canonical Rust path: `slatec_sys::linear_algebra::sparse::ssjac`
- Original SLATEC routine: `SSJAC`
- Native symbol: `ssjac_`
...
- Exact Netlib source file: [SSJAC](https://www.netlib.org/slatec/lin/ssjac.f)
