...
This canonical unsafe binding exposes original SLATEC routine `SSDCG`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSDCG](https://www.netlib.org/slatec/lin/ssdcg.f).
...
workspace `workspace` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (LENW). WORK     Real RWORK(LENW). Real array used for workspace. 5*N. not stated by selected source not applicable or not stated by selected source
...
- `RWORK`: WORK     Real RWORK(LENW). Real array used for workspace. 5*N.
...
- Original SLATEC routine: `SSDCG`
- Native symbol: `ssdcg_`
...
- Exact Netlib source file: [SSDCG](https://www.netlib.org/slatec/lin/ssdcg.f)
