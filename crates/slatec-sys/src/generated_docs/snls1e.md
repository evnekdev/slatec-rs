...
INTEGER IOPT,M,N,NPRINT,INFO,LWA INTEGER IW(N) REAL TOL REAL X(N),FVEC(M),WA(LWA) EXTERNAL FCN is an INTEGER work array of length N.

## `WA`

**Direction:** `input`. **Fortran type:** `REAL`. **Rust ABI type:** `*mut f32`. **Shape:** rank 1; dimensions (*).

INTEGER IOPT,M,N,NPRINT,INFO,LWA INTEGER IW(N) REAL TOL REAL X(N),FVEC(M),WA(LWA) EXTERNAL FCN is a work array of length LWA.
...
- `IW`: INTEGER IOPT,M,N,NPRINT,INFO,LWA INTEGER IW(N) REAL TOL REAL X(N),FVEC(M),WA(LWA) EXTERNAL FCN is an INTEGER work array of length N.
- `WA`: INTEGER IOPT,M,N,NPRINT,INFO,LWA INTEGER IW(N) REAL TOL REAL X(N),FVEC(M),WA(LWA) EXTERNAL FCN is a work array of length LWA.
