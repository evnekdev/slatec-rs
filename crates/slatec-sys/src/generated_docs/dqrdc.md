...
**Direction:** `input-output`. **Fortran type:** `DOUBLE PRECISION`. **Rust ABI type:** `*mut f64`. **Shape:** rank 2; dimensions (LDX, *).

DOUBLE PRECISION(LDX,P), where LDX. GE. N. contains the matrix whose decomposition is to be computed. X contains in its upper triangle the upper triangular matrix R of the QR factorization. Below its diagonal X contains information from which the orthogonal part of the decomposition can be recovered.
...
DOUBLE PRECISION(P). contains further information required to recover the orthogonal part of the decomposition.
...
DOUBLE PRECISION(P). is a work array. WORK is not referenced if.
