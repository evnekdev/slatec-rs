...
workspace `workspace` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (*). are set to ZERO, WORK(1) is set to A and WORK(LIMIT+1) to B. DIMENSIONING PARAMETERS ARRAYS form a decreasing sequence, with K = LAST if LAST.LE.(LIMIT/2+2), and Real Vector of dimension at least LENW on return contain the left contain the left end points of the subintervals in the end points of the subintervals in the partition of (A,B), partition of (A,B), ..., WORK(LIMIT+LAST) Contain the right end points, contain the contain the integral approximations over the subintervals, integral approximations over the subintervals, ..., WORK(LIMIT*3) contain the error estimates. are set to ZERO, WORK(1) is set to A and WORK(LIMIT+1) to B. DIMENSIONING PARAMETERS ARRAYS form a decreasing sequence, with K = LAST if LAST.LE.(LIMIT/2+2), and Real Vector of dimension at least LENW on return contain the left contain the left end points of the subintervals in the end points of the subintervals in the partition of (A,B), partition of (A,B), ..., WORK(LIMIT+LAST) Contain the right end points, contain the contain the integral approximations over the subintervals, integral approximations over the subintervals, ..., WORK(LIMIT*3) contain the error estimates. not applicable or not stated by selected source
