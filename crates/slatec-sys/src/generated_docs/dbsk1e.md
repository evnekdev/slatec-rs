...
input `scalar` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and scalar. Input positive double-precision argument. The function returns the exponentially scaled order-one modified Bessel K value at `X`; native code does not modify the scalar. not stated by selected source not applicable or not stated by selected source not a workspace argument
