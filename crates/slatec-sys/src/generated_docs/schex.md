...
REAL(LDR,P), where LDR. GE. P. contains the upper triangular factor that is to be updated. Elements of R below the diagonal are not referenced. contains the updated factor.
...
REAL(LDZ,NZ), where LDZ. GE. P. is an array of NZ P-vectors into which the transformation U is multiplied. Z is not referenced if NZ = 0. contains the updated matrix Z.
