...
is a triangular matrix. Here CTRANS(T) is the conjugate transpose. COMPLEX(LDT,N) contains the matrix of the system. The zero elements of the matrix are not referenced, and the corresponding elements of the array can be used to store other information.
...
COMPLEX(N). contains the right hand side of the system. B contains the solution, if INFO. EQ. 0. Otherwise B is unaltered.

## `JOB`

**Direction:** `input`. **Fortran type:** `INTEGER`. **Rust ABI type:** `*mut crate::FortranInteger`. **Shape:** scalar.

INTEGER specifies what kind of system is to be solved. If JOB is 00 solve T*X = B, T lower triangular, 01 solve T*X = B, T upper triangular, 10 solve CTRANS(T)*X = B, T lower triangular, 11 solve CTRANS(T)*X = B, T upper triangular.
