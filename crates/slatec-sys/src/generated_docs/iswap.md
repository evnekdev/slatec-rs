...
integer vector with N elements input vector IY (unchanged if N. LE. 0).
...
integer vector with N elements input vector IX (unchanged if N. LE. 0) Interchange integer IX and integer IY. For I = 0 to N-1, interchange IX(LX+I*INCX) and IY(LY+I*INCY), where LX = 1 if INCX. GE. 0, else LX = 1+(1-N)*INCX, and LY is defined in a similar way using INCY.
