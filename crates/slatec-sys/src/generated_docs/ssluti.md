...
This canonical unsafe binding exposes original SLATEC routine `SSLUTI`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSLUTI](https://www.netlib.org/slatec/lin/ssluti.f).
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (N). Readable sparse value array with at least `NELT` entries, stored in the SLAP column format described by `IA` and `JA`. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
workspace `workspace` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (10). Starting location of IL in IWORK. Starting location of JL in IWORK. Starting location of IU in IWORK. Starting location of JU in IWORK. Starting location of L in RWORK. Starting location of DINV in RWORK. Starting location of U in RWORK. See the DESCRIPTION of SSLUI4 for details. not stated by selected source not applicable or not stated by selected source
...
- Original SLATEC routine: `SSLUTI`
- Native symbol: `ssluti_`
...
- Exact Netlib source file: [SSLUTI](https://www.netlib.org/slatec/lin/ssluti.f)
