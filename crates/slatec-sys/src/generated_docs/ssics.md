...
This canonical unsafe binding exposes original SLATEC routine `SSICS`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSICS](https://www.netlib.org/slatec/lin/ssics.f).
...
input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (NEL). OUT      Real     EL(NEL). contain the unit lower triangular factor  of the incomplete decomposition   of the A  matrix  stored  in  SLAP Row format.   The Diagonal of   ones   *IS*   stored.     See "Description", below for more details about the SLAP Row fmt. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 11. `D`

input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (N). trans, of a symmetric positive definite matrix, A, which is stored in SLAP Column format.  The unit lower triangular matrix L is stored by rows, and the inverse of the diagonal matrix D is stored. OUT      Real D(N) 1./DIAG(A). not stated by selected source not applicable or not stated by selected source not a workspace argument

## 12. `R`

input-output `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (N). WORK     Real R(N). Temporary real workspace needed for the factorization. not stated by selected source not applicable or not stated by selected source
...
- `R`: WORK     Real R(N). Temporary real workspace needed for the factorization.
- `IWARN`: not a workspace argument

# ABI notes

- Canonical Rust path: `slatec_sys::linear_algebra::dense::ssics`
- Original SLATEC routine: `SSICS`
- Native symbol: `ssics_`
- ABI fingerprint: `subroutine:void(mut_i32,mut_i32,mut_i32_ptr_rank1,mut_i32_ptr_rank1,mut_f32_ptr_rank1,mut_i32,mut_i32,mut_i32_ptr_rank1,mut_i32_ptr_rank1,mut_f32_ptr_rank1,mut_f32_ptr_rank1,mut_f32_ptr_rank1,mut_i32)`
- Exact Netlib source file: [SSICS](https://www.netlib.org/slatec/lin/ssics.f)
