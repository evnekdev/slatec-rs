...
Approximates a definite integral over `(A, B)` with an adaptive Gauss-Kronrod rule. This is the original double precision QUADPACK driver `DQAG`.

# Description

`DQAG` repeatedly evaluates `F` and subdivides `(A, B)` until the requested absolute or relative accuracy is met, or it reports the limiting condition through `IER`. The selected source is [DQAG](https://www.netlib.org/slatec/src/dqag.f).

# Arguments

## `F`

**Direction:** `callback`. The synchronous `IntegrandF64` integrand receives one readable `f64` abscissa and returns its function value. It must remain valid through the call, must not retain the pointer, and **must not unwind** through Fortran.
...
**Direction:** `workspace-output`. `f64` array of at least `LENW` elements. On return its four `LIMIT`-strided segments hold left endpoints, right endpoints, subintegral estimates, and error estimates for the first `LAST` subintervals.
...
`F` is invoked synchronously using the reviewed `IntegrandF64` ABI. It may read its one scalar input only for the duration of the call and **must not panic or unwind** across the native boundary.
