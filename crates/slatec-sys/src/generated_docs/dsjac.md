...
workspace `workspace` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (LENW). WORK     Double Precision RWORK(LENW). Double Precision array used for workspace. 4*N. Upon return the following locations of IWORK hold information which may be of use to the user: not stated by selected source not applicable or not stated by selected source

## 17. `LENW`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. 4*N. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 18. `IWORK`

workspace `workspace` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (LENIW). WORK     Integer IWORK(LENIW). Used to hold pointers into the double precision workspace, Amount of Integer workspace actually used. Amount of Double Precision workspace actually used. 10. not stated by selected source not applicable or not stated by selected source
...
- `RWORK`: WORK     Double Precision RWORK(LENW). Double Precision array used for workspace. 4*N. Upon return the following locations of IWORK hold information which may be of use to the user:
- `LENW`: not a workspace argument
- `IWORK`: WORK     Integer IWORK(LENIW). Used to hold pointers into the double precision workspace, Amount of Integer workspace actually used. Amount of Double Precision workspace actually used. 10.
- `LENIW`: not a workspace argument

# ABI notes

- Canonical Rust path: `slatec_sys::linear_algebra::sparse::dsjac`
- Original SLATEC routine: `DSJAC`
- Native symbol: `dsjac_`
...
- Exact Netlib source file: [DSJAC](https://www.netlib.org/slatec/lin/dsjac.f)
