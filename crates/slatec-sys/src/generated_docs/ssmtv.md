...
X(N). The vector that should be multiplied by the transpose of the matrix.

## `Y`

**Direction:** `output`. **Fortran type:** `REAL`. **Rust ABI type:** `*mut f32`. **Shape:** rank 1; dimensions (N).

A'*X, where ' denotes transpose. Y(N). The product of the transpose of the matrix and the vector.
