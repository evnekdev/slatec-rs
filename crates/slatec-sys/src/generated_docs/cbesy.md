...
A parameter to indicate the scaling option 1 returns 2 (the underflows may not be in an uninterrupted sequence).
...
exp(-abs(Y))*Y(FNU+L-1,Z), L=1,. ,N, Y=Im(Z) which remove the exponential growth in both the upper and lower half planes as Z goes to infinity. Definitions and notation are found in the NBS Handbook of Mathematical Functions (Ref. 1). =2 returns Y(FNU+L-1,Z)*exp(-abs(Y)), L=1,. ,N where Y=Im(Z) Result vector of type COMPLEX.
...
**Direction:** `workspace-output`. **Fortran type:** `COMPLEX`. **Rust ABI type:** `*mut crate::Complex32`. **Shape:** rank 1; dimensions (N).

A work vector of type COMPLEX and dimension N.
