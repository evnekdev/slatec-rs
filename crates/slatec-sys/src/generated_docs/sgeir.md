...
REAL(LDA,N) the doubly subscripted array with dimension (LDA,N) which contains the coefficient matrix. A is not altered by the routine.
...
REAL(N*(N+1)) a singly subscripted array of dimension at least N*(N+1).
