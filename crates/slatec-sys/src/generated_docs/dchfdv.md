...
DCHFDV: Cubic Hermite Function and Derivative Evaluator Evaluates the cubic polynomial determined by function values

# Description

This canonical unsafe binding exposes original SLATEC routine `DCHFDV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DCHFDV](https://www.netlib.org/slatec/pchip/dchfdv.f).
...
input `scalar` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and scalar. and derivatives D1,D2 on interval (X1,X2), together with (input) values of function at X1 and X2, respectively. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
output `array` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (*). (output) real*8 array of values of the first derivative of the same function at the points  XE. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input-output `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. (output) error flag. Normal return: 0  (no errors). "Recoverable" errors: 1  if NE.LT.1 . 2  if X1.EQ.X2 . (Output arrays have not been changed in either case.) not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- Original SLATEC routine: `DCHFDV`
- Native symbol: `dchfdv_`
- ABI fingerprint: `subroutine:void(mut_f64,mut_f64,mut_f64,mut_f64,mut_f64,mut_f64,mut_i32,mut_f64_ptr_rank1,mut_f64_ptr_rank1,mut_f64_ptr_rank1,mut_i32_ptr_rank1,mut_i32)`
- Exact Netlib source file: [DCHFDV](https://www.netlib.org/slatec/pchip/dchfdv.f)
