...
This canonical unsafe binding exposes original SLATEC routine `STRMV`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [STRMV](https://www.netlib.org/slatec/lin/strmv.f).
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 2; dimensions (LDA, *). unit, upper or lower triangular matrix. Parameters ========== is unit triangular as follows: REAL             array of DIMENSION ( LDA, n). is not referenced. is not referenced. are not referenced either, but are assumed to be unity. Unchanged on exit. unit, upper or lower triangular matrix. Parameters ========== is unit triangular as follows: REAL             array of DIMENSION ( LDA, n). is not referenced. is not referenced. are not referenced either, but are assumed to be unity. Unchanged on exit. not applicable or not stated by selected source not a workspace argument
...
- Native symbol: `strmv_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [STRMV](https://www.netlib.org/slatec/lin/strmv.f)
