...
This canonical unsafe binding exposes original SLATEC routine `DQAWC`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DQAWC](https://www.netlib.org/slatec/src/dqawc.f).
...
input `scalar` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and scalar. Double precision Under limit of integration LIMIT.GE.1. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
input `scalar` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and scalar. Parameter in the weight function, C.NE.A, C.NE.B. A or C = B, the routine will end with A or C = B or not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- Original SLATEC routine: `DQAWC`
- Native symbol: `dqawc_`
- ABI fingerprint: `unavailable`
- Exact Netlib source file: [DQAWC](https://www.netlib.org/slatec/src/dqawc.f)
