...
Usage: INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX INTEGER ITER, IERR, IUNIT, LENW, IWORK(10), LENIW DOUBLE PRECISION B(N), X(N), A(NELT), TOL, ERR, RWORK(8*N) CALL DSDCGS(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL, $ ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW ) This routine performs preconditioned BiConjugate gradient method on the Non-Symmetric positive definite linear system

# Description

This canonical unsafe binding exposes original SLATEC routine `DSDCGS`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSDCGS](https://www.netlib.org/slatec/lin/dsdcgs.f).
...
workspace `workspace` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (LENW). WORK     Double Precision RWORK(LENW). Double Precision array used for workspace. 8*N. not stated by selected source not applicable or not stated by selected source

## 17. `LENW`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. 8*N. not stated by selected source not applicable or not stated by selected source not a workspace argument

## 18. `IWORK`

workspace `workspace` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (LENIW). WORK     Integer IWORK(LENIW). Used to hold pointers into the RWORK array. Upon return the following locations of IWORK hold information which may be of use to the user: Amount of Integer workspace actually used. Amount of Double Precision workspace actually used. 10. not stated by selected source not applicable or not stated by selected source
...
- `RWORK`: WORK     Double Precision RWORK(LENW). Double Precision array used for workspace. 8*N.
- `LENW`: not a workspace argument
- `IWORK`: WORK     Integer IWORK(LENIW). Used to hold pointers into the RWORK array. Upon return the following locations of IWORK hold information which may be of use to the user: Amount of Integer workspace actually used. Amount of Double Precision workspace actually used. 10.
- `LENIW`: not a workspace argument

# ABI notes

- Canonical Rust path: `slatec_sys::linear_algebra::sparse::dsdcgs`
- Original SLATEC routine: `DSDCGS`
- Native symbol: `dsdcgs_`
...
- Exact Netlib source file: [DSDCGS](https://www.netlib.org/slatec/lin/dsdcgs.f)
