...
This canonical unsafe binding exposes original SLATEC routine `SSLLTI`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [SSLLTI](https://www.netlib.org/slatec/lin/ssllti.f).
...
input `array` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and rank 1; dimensions (*). Writable output vector of length `N`. The interface computes the lower-triangular preconditioner solve `L^-1 * B` into this storage. Writable output vector of length `N`. The interface computes the lower-triangular preconditioner solve `L^-1 * B` into this storage. not applicable or not stated by selected source not a workspace argument
...
workspace `workspace` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and rank 1; dimensions (*). NEL Starting location of IEL in IWORK. Starting location of JEL in IWORK. Starting location of EL in RWORK. Starting location of DINV in RWORK. See the DESCRIPTION of SLLTI2 for details. not stated by selected source not applicable or not stated by selected source
...
- Canonical Rust path: `slatec_sys::linear_algebra::dense::ssllti`
- Original SLATEC routine: `SSLLTI`
- Native symbol: `ssllti_`
- ABI fingerprint: `subroutine:void(mut_i32,mut_f32_ptr_rank1,mut_f32_ptr_rank1,mut_i32,mut_i32_ptr_rank1,mut_i32_ptr_rank1,mut_f32_ptr_rank1,mut_i32,mut_f32_ptr_rank1,mut_i32_ptr_rank1)`
- Exact Netlib source file: [SSLLTI](https://www.netlib.org/slatec/lin/ssllti.f)
