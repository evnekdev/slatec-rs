...
input `scalar` argument; Fortran declaration `COMPLEX`, Rust ABI type `*mut crate::Complex32`, and scalar. calculates the complex cube root of Z.  The principal root for which -PI .LT. arg(Z) .LE. +PI is returned. not stated by selected source not applicable or not stated by selected source not a workspace argument
