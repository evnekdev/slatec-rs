...
This canonical unsafe binding exposes original SLATEC routine `DSDCG`. Its documented behavior is taken from the selected, source-hash-verified provider prologue; the exact implementation source is [DSDCG](https://www.netlib.org/slatec/lin/dsdcg.f).
...
workspace `workspace` argument; Fortran declaration `DOUBLE PRECISION`, Rust ABI type `*mut f64`, and rank 1; dimensions (LENW). WORK     Double Precision RWORK(LENW). Double Precision array used for workspace. 5*N. Upon return the following locations of IWORK hold information which may be of use to the user: not stated by selected source not applicable or not stated by selected source

## 17. `LENW`

input `scalar` argument; Fortran declaration `INTEGER`, Rust ABI type `*mut crate::FortranInteger`, and scalar. IN       Integer. 5*N. not stated by selected source not applicable or not stated by selected source not a workspace argument
...
- Original SLATEC routine: `DSDCG`
- Native symbol: `dsdcg_`
...
- Exact Netlib source file: [DSDCG](https://www.netlib.org/slatec/lin/dsdcg.f)
