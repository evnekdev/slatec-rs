...
input `scalar` argument; Fortran declaration `REAL`, Rust ABI type `*mut f32`, and scalar. calculates the double precision reciprocal of the complete Gamma function for double precision argument X. not stated by selected source not applicable or not stated by selected source not a workspace argument
