...
Carrier status: `ready`. The carrier distributes one `accepted-source-closure` archive. A family is available only when its entire exact source closure is recorded in that archive; its 129 hash-pinned historical source units have source-specific `explicit-public-domain` records in the generated provenance inventory. This is an evidence record, not legal advice.
